...
type LangSymbol* = object ## symbol signature in Nim
...
func nimIdentBackticksNormalize*(s: string): string =
  ...
  for i in 0 .. len(s) - 1:
    if s[i] in {'A' .. 'Z'}:
      ...
      discard # just omit '`' or ' '
  if j != s.len:
    setLen(result, j)

proc langSymbolGroup*(kind: string, name: string): LangSymbol =
  if kind notin ["proc", "func", "macro", "method", "iterator", "template", "converter"]:
...
proc toLangSymbol*(linkText: PRstNode): LangSymbol =
  ...
  const NimDefs = [
    "proc", "func", "macro", "method", "iterator", "template", "converter", "const",
    "type", "var", "enum", "object", "tuple", "module",
  ]
  ...
  template flushIdent() =
    if curIdent != "":
      case state
      of inBeginning:
        fail("incorrect state inBeginning")
      of afterSymKind:
        resolveSymKind curIdent
      of beforeSymbolName:
        fail("incorrect state beforeSymbolName")
      of atSymbolName:
        result.name = curIdent.nimIdentBackticksNormalize
      of afterSymbolName:
        fail("incorrect state afterSymbolName")
      of genericsPar:
        result.generics = curIdent
      of parameterName:
        result.parameters.add (curIdent, "")
      of parameterType:
        for a in countdown(result.parameters.len - 1, 0):
          if result.parameters[a].`type` == "":
            result.parameters[a].`type` = curIdent
      of outType:
        result.outType = curIdent
      curIdent = ""

  var parens = 0
  let L = linkText.sons.len
  template s(i: int): string =
    linkText.sons[i].text
  ...
  template nextState() =
    ...
    of ":":
      ...
      let isPostfixSymKind =
        i > 0 and i == L - 1 and result.symKind == "" and s(i) in NimDefs
