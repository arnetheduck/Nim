...
type SignalContextCapability* {.
  importcpp: "Genode::Signal_context_capability", header: "<base/signal.h>", pure
.} = object ## Capability to an asynchronous signal context.
...
type
  ...
  SignalHandlerBase {.
    importcpp: "Nim::SignalHandler", header: "genode_cpp/signals.h", pure
  .} = object
  ...
  SignalHandlerObj = object
    ...
    cb: HandlerProc ## Signal handling procedure called during dispatch.

  SignalHandler* = ref SignalHandlerObj ## Nim object enclosing a Genode signal handler.
