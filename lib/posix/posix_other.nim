...
type DIR* {.importc: "DIR", header: "<dirent.h>", incompleteStruct.} = object
  ## A type representing a directory stream.

type SocketHandle* = distinct cint # The type used to represent socket descriptors

type
  Time* {.importc: "time_t", header: "<time.h>".} =
    distinct (when defined(nimUse64BitCTime): int64 else: clong)
  ...
  Dirent* {.importc: "struct dirent", header: "<dirent.h>", final, pure.} = object
    ...
    when defined(dragonfly):
      ...
      d_type*: uint8
        ...
        defined(openbsd) or defined(genode):
  ...
  Glob* {.importc: "glob_t", header: "<glob.h>", final, pure.} = object ## glob_t
  ...
  Posix_typed_mem_info* {.
    importc: "struct posix_typed_mem_info", header: "<sys/mman.h>", final, pure
  .} = object
    posix_tmi_length*: int

  Tm* {.importc: "struct tm", header: "<time.h>", final, pure.} = object ## struct tm
  ...
  Stack* {.importc: "stack_t", header: "<signal.h>", final, pure.} = object ## stack_t
...
when hasAioH:
  type Taiocb* {.importc: "struct aiocb", header: "<aio.h>", final, pure.} = object
    ## struct aiocb
...
when defined(linux):
  ...
  when defined(net_ipv4) or defined(net_ipv6) or defined(net_raw):
    {.
      emit: [
        "NIM_STATIC_ASSERT(NET_SOCKADDR_MAX_SIZE == ", Sockaddr_max_length,
...
when defined(lwip):
  type
    SockAddr* {.importc: "struct sockaddr", header: "<sys/socket.h>", pure, final.} = object
      ...
      sa_data*:
        array[0 .. Sockaddr_max_length - sizeof(uint8) - sizeof(TSa_Family), char]
        ## Socket address (variable-length data).

    Sockaddr_storage* {.
      importc: "struct sockaddr_storage", header: "<sys/socket.h>", pure, final
    .} = object ## struct sockaddr_storage
  ...
  type
    SockAddr* {.importc: "struct sockaddr", header: "<sys/socket.h>", pure, final.} = object
      ## struct sockaddr
      sa_family*: TSa_Family ## Address family.
      data*: array[0 .. Sockaddr_max_length - sizeof(TSa_Family), char]
        ## Socket address (variable-length data).
  ...
  {.
    emit: [
      "NIM_STATIC_ASSERT(sizeof(struct sockaddr) == ",
      sizeof(Sockaddr),
  ...
  {.
    emit: [
      "NIM_STATIC_ASSERT(sizeof(struct sockaddr_storage) == ",
  ...
  type
    SockAddr* {.importc: "struct sockaddr", header: "<sys/socket.h>", pure, final.} = object
      ## struct sockaddr
      sa_family*: TSa_Family ## Address family.
      sa_data*: array[0 .. Sockaddr_max_length - sizeof(TSa_Family), char]
        ## Socket address (variable-length data).
...
type Sockaddr_un* {.importc: "struct sockaddr_un", header: "<sys/un.h>", pure, final.} = object
  ...
  sun_path*: array[0 .. Sockaddr_un_path_length - sizeof(TSa_Family), char]
    ## Socket path
...
type
  ...
  Sockaddr_in6* {.
    importc: "struct sockaddr_in6", pure, final, header: "<netinet/in.h>"
  .} = object ## struct sockaddr_in6
...
when not defined(lwip):
  type TPollfd* {.importc: "struct pollfd", pure, final, header: "<poll.h>".} = object
    ## struct pollfd
...
when defined(linux):
  var MAP_POPULATE* {.importc, header: "<sys/mman.h>".}: cint
    ## Populate (prefault) page tables for a mapping.
else:
  var MAP_POPULATE*: cint = 0

when defined(linux) or defined(nimdoc):
  when defined(alpha) or defined(mips) or defined(mipsel) or defined(mips64) or
      defined(mips64el) or defined(parisc) or defined(sparc) or defined(sparc64) or
      defined(nimdoc):
...
when defined(macosx):
  ...
  const MSG_NOSIGNAL* = 0'i32
  var SO_NOSIGPIPE* {.importc, header: "<sys/socket.h>".}: cint
...
when defined(haiku):
  const SIGKILLTHR* = 21 ## BeOS specific: Kill just the thread, not team
