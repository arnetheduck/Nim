...
type
  ...
  .} = object ## struct sockaddr_storage
    ...
    ss_align: clonglong
    ss_padding2: array[
      128 - sizeof(uint8) - sizeof(cshort) - (64 - sizeof(uint8) - sizeof(cshort)) - 64,
      char,
    ]
  ...
  IOVec* {.importc: "struct iovec", pure, final, header: "<sys/socket.h>".} = object
    ## struct iovec
...
proc WIFSIGNALED*(s: cint): bool =
  ((s and 0x7f) > 0) and ((s and 0x7f) < 0x7f)
