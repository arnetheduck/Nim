...
type
  ...
  Pthread_barrier* {.
    importc: "pthread_barrier_t", header: "<sys/types.h>", pure, final
  .} = object
    abi: array[32 div sizeof(clong), clong]

  Pthread_barrierattr* {.
    importc: "pthread_barrierattr_t", header: "<sys/types.h>", pure, final
  .} = object
  ...
  Pthread_condattr* {.
    importc: "pthread_condattr_t", header: "<sys/types.h>", pure, final
  .} = object
    abi: array[4 div sizeof(cint), cint]
  ...
  Pthread_mutexattr* {.
    importc: "pthread_mutexattr_t", header: "<sys/types.h>", pure, final
  .} = object
    abi: array[4 div sizeof(cint), cint]
...
template sa_sigaction*(
    v: Sigaction
): proc(x: cint, y: ptr SigInfo, z: pointer) {.noconv.} =
  cast[proc(x: cint, y: ptr SigInfo, z: pointer) {.noconv.}](v.sa_handler)

proc `sa_sigaction=`*(
    v: var Sigaction, x: proc(x: cint, y: ptr SigInfo, z: pointer) {.noconv.}
) =
...
type
  ...
  .} = object ## struct sockaddr_storage
    ...
    ss_padding {.importc: "__ss_padding".}:
      array[128 - sizeof(cshort) - sizeof(culong), char]
...
# <sys/wait.h>
proc WEXITSTATUS*(s: cint): cint =
  (s and 0xff00) shr 8

proc WTERMSIG*(s: cint): cint =
  s and 0x7f

proc WSTOPSIG*(s: cint): cint =
  WEXITSTATUS(s)

proc WIFEXITED*(s: cint): bool =
  WTERMSIG(s) == 0

proc WIFSIGNALED*(s: cint): bool =
  (cast[int8]((s and 0x7f) + 1) shr 1) > 0

proc WIFSTOPPED*(s: cint): bool =
  (s and 0xff) == 0x7f

proc WIFCONTINUED*(s: cint): bool =
  s == WCONTINUED
