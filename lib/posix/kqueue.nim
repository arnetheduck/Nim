...
when defined(macosx) or defined(freebsd) or defined(openbsd) or defined(dragonfly):
  const
    EVFILT_READ* = -1
    EVFILT_WRITE* = -2
...
const
  EV_ADD* = 0x0001
    ## Add event to queue (implies enable).
...
const NOTE_LOWAT* = 0x0001 ## low water mark
...
when defined(macosx) or defined(freebsd):
  ...
  const NOTE_MSECONDS* = 0x00000000'u32
...
proc EV_SET*(
  ...
  data: int,
  udata: pointer,
) {.importc: "EV_SET", header: "<sys/event.h>".} ## Fills event with given data.
