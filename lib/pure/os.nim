...
const ExeExts* = ## Platform specific file extension for executables.
...
when supportedSystem:
  proc findExe*(
      exe: string, followSymlinks: bool = true, extensions: openArray[string] = ExeExts
  ): string {.tags: [ReadDirEffect, ReadEnvEffect, ReadIOEffect], noNimJs.} =
  ...
  when weirdTarget:
    ...
    template Time(x: untyped): untyped =
      string

  proc getLastModificationTime*(
      file: string
  ): times.Time {.rtl, extern: "nos$1", noWeirdTarget.} =
  ...
  proc expandFilename*(
      filename: string
  ): string {.rtl, extern: "nos$1", tags: [ReadDirEffect], noWeirdTarget.} =
  ...
  proc sleep*(
      milsecs: int
  ) {.rtl, extern: "nos$1", tags: [TimeEffect], noWeirdTarget.} =
  ...
  proc getFileSize*(
      file: string
  ): BiggestInt {.rtl, extern: "nos$1", tags: [ReadIOEffect], noWeirdTarget.} =
  ...
  proc execShellCmd*(
      command: string
  ): int {.rtl, extern: "nos$1", tags: [ExecIOEffect], noWeirdTarget.} =
...
when not weirdTarget and
    (defined(linux) or defined(solaris) or defined(bsd) or defined(aix)):
...
when defined(macosx):
  ...
  proc getExecPath1(
    c: cstring, size: var cuint32
  ) {.importc: "_NSGetExecutablePath", header: "<sys/param.h>".}

  proc getExecPath2(
    c: cstring, size: var cuint32
  ): bool {.importc: "_NSGetExecutablePath", header: "<mach-o/dyld.h>".}
...
when supportedSystem:
  proc getAppFilename*(): string {.
      rtl, extern: "nos$1", tags: [ReadIOEffect], noWeirdTarget, raises: []
  .} =
...
when weirdTarget or defined(windows) or defined(posix) or defined(nintendoswitch):
  type FileInfo* = object
    ...
    blockSize*: int
      ## Preferred I/O block size for this object.
      ## In some filesystems, this may vary from file to file.
    isSpecial*: bool
      ## Is file special? (on Unix some "files"
  ...
  template rawToFormalFileInfo(rawInfo, path, formalInfo): untyped =
    ...
    when defined(windows):
      template merge[T](a, b): untyped =
        cast[T]((uint64(cast[uint32](a))) or (uint64(cast[uint32](b)) shl 32))
  ...
  proc sameFileContent*(
      path1, path2: string
  ): bool {.rtl, extern: "nos$1", tags: [ReadIOEffect], noWeirdTarget.} =
  ...
  proc setLastModificationTime*(file: string, t: times.Time) {.noWeirdTarget.} =
    ...
    when defined(posix):
      ...
      var timevals =
        [Timeval(tv_sec: unixt, tv_usec: micro), Timeval(tv_sec: unixt, tv_usec: micro)]
        # [last access, last modification]
      if utimes(file, timevals.addr) != 0:
        raiseOSError(osLastError(), file)
    else:
...
func isValidFilename*(filename: string, maxLen = 259.Positive): bool {.since: (1, 1).} =
  ...
  if unlikely(
    f.name.len + f.ext.len > maxLen or f.name.len == 0 or f.name[0] == ' ' or
      f.name[^1] == ' ' or f.name[^1] == '.' or find(f.name, invalidFilenameChars) != -1
  ):
    return false
  for invalid in invalidFilenames:
    if cmpIgnoreCase(f.name, invalid) == 0:
      return false
...
when not weirdTarget:
  ...
  ): untyped {.deprecated: "use fileExists".} =
    fileExists(args)
