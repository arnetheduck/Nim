...
when defined(windows):
  ...
  proc eqEncodingNames(a, b: string): bool =
    ...
    while i < a.len and j < b.len:
      ...
      if b[j] in {'-', '_'}:
        inc j
      if i < a.len and j < b.len and a[i].toLowerAscii != b[j].toLowerAscii:
        return false
      inc i
      inc j
  ...
  when false:
    ...
    proc getCPInfo(
      codePage: CodePage, lpCPInfo: var CpInfo
    ): int32 {.stdcall, importc: "GetCPInfo", dynlib: "kernel32".}

  proc nameToCodePage*(name: string): CodePage =
    var nameAsInt: int
    if parseInt(name, nameAsInt) == 0:
      nameAsInt = -1
    for no, na in items(winEncodings):
      if no == nameAsInt or eqEncodingNames(na, name):
        return CodePage(no)
    result = CodePage(-1)
  ...
  proc getGetConsoleCP(): CodePage {.
    stdcall, importc: "GetConsoleCP", dynlib: "kernel32"
  .}
  ...
  proc multiByteToWideChar(
    ...
    cchWideChar: cint,
  ): cint {.stdcall, importc: "MultiByteToWideChar", dynlib: "kernel32".}
  ...
  proc wideCharToMultiByte(
    ...
    lpUsedDefaultChar: pointer = nil,
  ): cint {.stdcall, importc: "WideCharToMultiByte", dynlib: "kernel32".}
  ...
  proc iconvOpen(
    tocode, fromcode: cstring
  ): EncodingConverter {.importc: "iconv_open", importIconv.}

  proc iconvClose(c: EncodingConverter) {.importc: "iconv_close", importIconv.}
  proc iconv(
    c: EncodingConverter,
    inbuf: ptr cstring,
    inbytesLeft: ptr csize_t,
    outbuf: ptr cstring,
    outbytesLeft: ptr csize_t,
  ): csize_t {.importc: "iconv", importIconv.}
...
proc open*(destEncoding = "UTF-8", srcEncoding = "CP1252"): EncodingConverter =
  ...
  when not defined(windows):
    ...
    if result == cast[EncodingConverter](-1):
      raise newException(
        EncodingError,
        "cannot create encoding converter from " & srcEncoding & " to " & destEncoding,
      )
  else:
    result.dest = nameToCodePage(destEncoding)
    result.src = nameToCodePage(srcEncoding)
    if int(result.dest) == -1:
      raise newException(EncodingError, "cannot find encoding " & destEncoding)
    if int(result.src) == -1:
      raise newException(EncodingError, "cannot find encoding " & srcEncoding)
...
when defined(windows):
  ...
  proc convertWin(codePageFrom: CodePage, codePageTo: CodePage, s: string): string =
    ...
    if int(codePageFrom) in unsupported:
      let message =
        "encoding from " & codePageToName(codePageFrom) & " is not supported on windows"
      raise newException(EncodingError, message)

    if int(codePageTo) in unsupported:
      let message =
        "encoding to " & codePageToName(codePageTo) & " is not supported on windows"
      raise newException(EncodingError, message)

    # in case it's already UTF-16 little endian - conversion can be simplified
    let wideString =
      if int(codePageFrom) == 1200:
        s
      else:
        convertToWideString(codePageFrom, s)
    return
      if int(codePageTo) == 1200:
        wideString
      else:
        convertFromWideString(codePageTo, wideString)
