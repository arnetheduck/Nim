...
proc unregister*[T](s: Selector[T], fd: int | SocketHandle) =
  ...
  if pkey.events != {}:
    when not defined(android):
      if Event.Read in pkey.events or Event.Write in pkey.events or
          Event.User in pkey.events:
        var epv = EpollEvent()
...
when not defined(android):
  ...
  proc registerProcess*[T](s: Selector, pid: int, data: T): int {.discardable.} =
    var
      nmask: Sigset
      omask: Sigset
...
proc selectInto*[T](
  ...
  let count = epoll_wait(s.epollFD, addr(resTable[0]), maxres.cint, timeout.cint)
  if count < 0:
    result = 0
    let err = osLastError()
    if cint(err) != EINTR:
    ...
    while i < count:
      ...
      if (pevents and EPOLLERR) != 0 or (pevents and EPOLLHUP) != 0:
        if (pevents and EPOLLHUP) != 0:
          ...
          if getsockopt(SocketHandle fdi, SOL_SOCKET, SO_ERROR, addr(error), addr(size)) ==
              0'i32:
      ...
      when not defined(android):
        if (pevents and EPOLLIN) != 0:
          if Event.Read in pkey.events:
            ...
            var data: uint64 = 0
            if posix.read(cint(fdi), addr data, sizeof(uint64)) != sizeof(uint64):
              raiseIOSelectorsError(osLastError())
