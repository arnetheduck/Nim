...
import std/[pcre, strutils, rtarrays]

when defined(nimPreviewSlimSystem):
  import std/syncio

const MaxSubpatterns* = 20
...
type
  RegexFlag* = enum ## options for regular expressions
    ...
    reStudy = 4
      ## study the expression (may be omitted if the
...
proc matchOrFind(
  ...
  var
    ...
    res = pcre.exec(
      ...
      start,
      flags,
      cast[ptr cint](rawMatches),
      (matches.len + 1).cint * 3,
    )
  if res < 0'i32:
    return res
...
proc findBounds*(
    s: string, pattern: Regex, matches: var openArray[string], start = 0
): tuple[first, last: int] {.inline.} =
  ...
  result = findBounds(
    cstring(s), pattern, matches, min(start, MaxReBufSize), min(s.len, MaxReBufSize)
  )
...
proc findBoundsImpl(
    buf: cstring, pattern: Regex, start = 0, bufSize = 0, flags = 0
): tuple[first, last: int] =
...
proc findBounds*(
    buf: cstring, pattern: Regex, start = 0, bufSize: int
): tuple[first, last: int] =
...
proc findBounds*(
  ...
  result =
    findBounds(cstring(s), pattern, min(start, MaxReBufSize), min(s.len, MaxReBufSize))
...
proc matchLen*(
  ...
  ## .. note:: The memory for `matches` needs to be allocated before this function is called, otherwise it will just remain empty.
  result =
    matchOrFind(cstring(s), pattern, matches, start.cint, s.len.cint, pcre.ANCHORED)
...
iterator findAll*(s: string, pattern: Regex, start = 0): string =
  ...
  while true:
    let res = pcre.exec(
      pattern.h, pattern.e, s, len(s).cint, i, 0'i32, cast[ptr cint](rawMatches), 3
    )
    if res < 0'i32:
      break
    let a = rawMatches[0]
...
iterator findAll*(buf: cstring, pattern: Regex, start = 0, bufSize: int): string =
  ...
  while true:
    let res = pcre.exec(
      pattern.h, pattern.e, buf, bufSize.cint, i, 0'i32, cast[ptr cint](rawMatches), 3
    )
    if res < 0'i32:
      break
    let a = rawMatches[0]
...
proc findAll*(s: string, pattern: Regex, start = 0): seq[string] {.inline.} =
  ...
  result = @[]
  for x in findAll(s, pattern, start):
    result.add x
...
proc replacef*(s: string, sub: Regex, by: string): string =
  ...
  while prev < s.len:
    ...
    add(result, substr(s, prev, match.first - 1))
    addf(result, by, caps)
    if match.last + 1 == prev:
      break
    prev = match.last + 1
  add(result, substr(s, prev))

proc multiReplace*(
    s: string, subs: openArray[tuple[pattern: Regex, repl: string]]
): string =
...
proc transformFile*(
    infile, outfile: string, subs: openArray[tuple[pattern: Regex, repl: string]]
) =
