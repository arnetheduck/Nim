...
proc fromJson*[T](a: var T, b: JsonNode, opt = Joptions()) =
  ...
  when compiles(fromJsonHook(a, b, opt)):
    ...
    of JString:
      ...
      checkJson false, "Expecting int/string for " & $T & " got " & b.pretty()
    ...
    of JString:
      ...
      checkJson false, "Expecting null/string for " & $T & " got " & b.pretty()
  elif T is JsonNode:
    a = b
  elif T is ref | ptr:
    if b.kind == JNull:
    ...
    when isNamedTuple(T):
      ...
      checkJson b.len == tupleSize,
        "Json doesn't match expected length of " & $tupleSize & ", got " & b.pretty()
      var i = 0
      for val in fields(a):
...
proc fromJsonHook*[A](s: var SomeSet[A], jsonNode: JsonNode, opt = Joptions()) =
  ...
  assert jsonNode.kind == JArray,
    "The kind of the `jsonNode` must be `JArray`, but its actual " & "type is `" &
      $jsonNode.kind & "`."
  clear(s)
  for v in jsonNode:
