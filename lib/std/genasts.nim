...
type GenAstOpt* = enum
  ...
  kNoNewLit # don't call call newLit automatically in `genAst` capture parameters
...
macro genAstOpt*(options: static set[GenAstOpt], args: varargs[untyped]): untyped =
  ...
  runnableExamples:
    ...
    if false:
      check2 a * 2 < a + 1
      # would error with: 'a * 2 < a + 1'' failed: lhs: '6', rhs: '4'
  ...
  runnableExamples:
    ...
    macro fun(a: string, b: static bool): untyped =
      ...
      proc localFun(): auto =
        12 # implicitly {.inject.}, doesn't need to be captured.
  ...
  template newLitMaybe(a): untyped =
    when (a is type) or (typeof(a) is (proc | iterator | proc | NimNode)):
  ...
  for a in args[0 ..^ 2]:
    ...
    of nnkIdent:
      ...
      error("invalid argument kind: " & $a.kind, a)
    if kNoNewLit notin options:
      varVal = newCall(bindSym"newLitMaybe", varVal)
  ...
  result.add nnkTemplateDef.newTree(
    name, newEmptyNode(), newEmptyNode(), params, pragmas, newEmptyNode(), args[^1]
  )
