...
from std/private/osfiles import
  fileExists, removeFile, moveFile, copyFile, copyFileWithPermissions, copyFileToDir,
  tryRemoveFile, getFilePermissions, setFilePermissions, CopyFlag, FilePermission

export CopyFlag, FilePermission

proc getFilePermissions*(
    filename: Path
): set[FilePermission] {.inline, tags: [ReadDirEffect].} =
...
proc setFilePermissions*(
    filename: Path, permissions: set[FilePermission], followSymlinks = true
...
proc copyFile*(
    source, dest: Path, options = cfSymlinkFollow, bufferSize = 16_384
) {.inline, tags: [ReadDirEffect, ReadIOEffect, WriteIOEffect].} =
...
proc copyFileWithPermissions*(
    source, dest: Path, ignorePermissionErrors = true, options = cfSymlinkFollow
) {.inline.} =
  ...
  ## See also:
  ## * `copyFile proc`_
  copyFileWithPermissions(source.string, dest.string, ignorePermissionErrors, {options})

proc copyFileToDir*(
    source, dir: Path, options = cfSymlinkFollow, bufferSize = 16_384
) {.inline.} =
