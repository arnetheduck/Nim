...
when defined(windows) and not weirdTarget:
  ...
  proc skipFindData*(f: WIN32_FIND_DATA): bool {.inline.} =
    ...
    const dot = ord('.')
    result =
      f.cFileName[0].int == dot and
      (f.cFileName[1].int == 0 or f.cFileName[1].int == dot and f.cFileName[2].int == 0)
...
when supportedSystem:
  ...
  proc tryMoveFSObject*(source, dest: string, isDir: bool): bool {.noWeirdTarget.} =
    ...
    when defined(windows):
      ...
      result =
        moveFileExW(s, d, MOVEFILE_COPY_ALLOWED or MOVEFILE_REPLACE_EXISTING) != 0'i32
    else:
  ...
  proc fileExists*(
      filename: string
  ): bool {.rtl, extern: "nos$1", tags: [ReadDirEffect], noNimJs, sideEffect.} =
  ...
  proc symlinkExists*(
      link: string
  ): bool {.rtl, extern: "nos$1", tags: [ReadDirEffect], noWeirdTarget, sideEffect.} =
