...
iterator walkDirRecFilter*(
    dir: string,
    follow: proc(entry: PathEntry): bool = nil,
    relative = false,
    checkDir = true,
): PathEntry {.tags: [ReadDirEffect], effectsOf: follow.} =
...
when isMainModule:
  import std/sugar
  for a in walkDirRecFilter(
    ".",
    follow =
      a =>
      a.path.lastPathPart notin ["nimcache", ".git", "csources_v1", "csources", "bin"],
  ):
    echo a
