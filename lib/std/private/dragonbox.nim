...
const dtoaMinBufferLength*: cint = 64
...
const
  ##   static constexpr int32_t   MaxExponent     = 1024 - 1 - (SignificandSize - 1);
...
const maxIeeeExponent*: BitsType = BitsType(2 * 1024 - 1)
...
const exponentMask*: BitsType = maxIeeeExponent shl (significandSize - 1)

const signMask*: BitsType = not (not BitsType(0) shr 1)
...
type uint64x2* {.bycopy.} = object
  hi*: uint64
  lo*: uint64

proc computePow10*(k: int32): uint64x2 {.inline.} =
  const kMin: int32 = -292
  const kMax: int32 = 326
  const pow10: array[kMax - kMin + 1, uint64x2] = [
...
proc multipleOfPow5*(value: uint64, e5: int32): bool {.inline.} =
  type MulCmp {.bycopy.} = object
    mul: uint64
    cmp: uint64

  const mod5 = [
    MulCmp(mul: 0x0000000000000001'u, cmp: 0xFFFFFFFFFFFFFFFF'u),
...
type FloatingDecimal64* {.bycopy.} = object
  significand*: uint64
  exponent*: int32
...
proc toDecimal64AsymmetricInterval*(e2: int32): FloatingDecimal64 {.inline.} =
  ...
  let lowerEndpoint: uint64 =
    (pow10.hi - (pow10.hi shr (P + 1))) shr (64 - P - betaMinus1)
  let upperEndpoint: uint64 =
    (pow10.hi + (pow10.hi shr (P + 0))) shr (64 - P - betaMinus1)
...
proc toDecimal64*(
    ieeeSignificand: uint64, ieeeExponent: uint64
): FloatingDecimal64 {.inline.} =
  const kappa: int32 = 2
  const bigDivisor: uint32 = 1000
  ##  10^(kappa + 1)
  const smallDivisor: uint32 = 100
  ##  10^(kappa)
  ##
  ##  Step 1:
...
proc utoa8DigitsSkipTrailingZeros*(
...
proc printDecimalDigitsBackwards*(
    buf: var openArray[char], pos: int, output64: uint64
): int {.inline.} =
  var pos = pos
  ...
  if output >= 10000:
    ...
    if r != 0:
      ...
      if tz == nd:
        inc(tz, trailingZeros2Digits(if rL == 0: rH else: rL) + (if rL == 0: 2 else: 0))
    else:
      if tz == nd:
        inc(tz, 4)
      else:
...
proc formatDigits*[T: Ordinal](
  ...
  const minFixedDecimalPoint = -6
  const maxFixedDecimalPoint = 17
  var pos: int = pos.int
  ...
  if useFixed:
    if decimalPoint <= 0:
      ...
      when true: #defined(vcc) and not defined(clang):
        ...
        for i in 0 ..< 16:
          tmp[i] = buffer[i + pos + decimalPoint]
        for i in 0 ..< 16:
          buffer[i + pos + decimalPoint + 1] = tmp[i]
      else:
...
proc toChars*(
    buffer: var openArray[char], v: float, forceTrailingDotZero = false
): int {.inline.} =
  var pos = 0
  ...
  if exponent != maxIeeeExponent:
    ...
    if exponent != 0 or significand != 0:
      ...
      return formatDigits(
        buffer, pos, dec.significand, dec.exponent.int, forceTrailingDotZero
      )
    else:
      buffer[pos] = '0'
      buffer[pos + 1] = '.'
