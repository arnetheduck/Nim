...
func entries*(
  self: Headers
): seq[tuple[key, value: cstring]] {.importjs: "Array.from(#.$1())".}
...
func toCstring*(
  self: Headers
): cstring {.importjs: "JSON.stringify(Array.from(#.entries()))".}
...
runnableExamples("-r:off"):
  block:
    ...
    assert header.entries() ==
      @[("key".cstring, "value".cstring), ("other".cstring, "another".cstring)]
