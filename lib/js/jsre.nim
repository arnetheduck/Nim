...
type RegExp* = ref object of JsRoot
  ...
  global*: bool
    ## Whether to test against all possible matches in a string, or only against the first.
...
runnableExamples:
  ...
  assert "abbcccdddd".replace(
    jsregex,
    proc(m: varargs[cstring]): cstring =
      ($m[0] & $(m.len)).cstring,
  ) == "a1b2c3d4"
