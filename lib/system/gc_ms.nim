...
template mulThreshold(x): untyped =
  x * 2
...
type
  WalkOp = enum
    waMarkGlobal
      # we need to mark conservatively for global marker procs
...
when not defined(nimSeqsV2):
  ...
  proc growObj(old: pointer, newsize: int, gch: var GcHeap): pointer =
    ...
    var oldsize =
      align(GenericSeqSize, elemAlign) + cast[PGenericSeq](old).len * elemSize
...
proc collectCT(gch: var GcHeap, size: int) =
  ...
  if (
    getOccupiedMem(gch.region) >= gch.cycleThreshold or
    size > fmem and fmem > InitialThreshold
  ) and gch.recGcLock == 0:
    collectCTBody(gch)
