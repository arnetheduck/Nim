...
when not defined(memTracker):
  {.
    error:
      "Memory tracking support is turned off! Enable memory tracking by passing `--memtracker:on` to the compiler (see the Nim Compiler User Guide for more options)."
  .}
...
var
  gLog*: TrackLog
  gLogger*: TrackLogger = proc(log: TrackLog) =
    discard
...
proc memTrackerWrite(
    address: pointer, size: int, file: cstring, line: int
) {.compilerproc.} =
  addEntry LogEntry(
    op: "write",
    address: address,
...
proc memTrackerOp*(op: cstring, address: pointer, size: int) {.tags: [], gcsafe.} =
  addEntry LogEntry(
    op: op, address: address, size: size, file: "", line: 0, thread: myThreadId()
  )
