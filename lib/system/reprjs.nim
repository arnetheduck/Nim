...
proc reprInt(x: uint64): string {.compilerproc.} =
  $x

proc reprInt(x: int): string {.compilerproc.} =
  $x
...
proc addSetElem(result: var string, elem: int, typ: PNimType) =
  ...
  # Note the base to advance towards the element type
...
proc reprArray(a: pointer, typ: PNimType, cl: var ReprClosure): string {.compilerRtl.} =
...
proc reprRecord(
    o: pointer, typ: PNimType, cl: var ReprClosure
): string {.compilerRtl.} =
