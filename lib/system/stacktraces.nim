...
when defined(nimStackTraceOverride):
  ...
  type
    ...
    StackTraceOverrideGetTracebackProc* =
    ...
    .}
    StackTraceOverrideGetDebuggingInfoProc* = proc(
      programCounters: seq[cuintptr_t], maxLength: cint
    ): seq[StackTraceEntry] {.nimcall, gcsafe, raises: [], tags: [], noinline.}
  ...
  var
    stackTraceOverrideGetTraceback: StackTraceOverrideGetTracebackProc = proc(): string {.
        nimcall, gcsafe, raises: [], tags: [], noinline
    .} =
      discard
      #result = "Stack trace override procedure not registered.\n"
    stackTraceOverrideGetProgramCounters: StackTraceOverrideGetProgramCountersProc = proc(
        maxLength: cint
    ): seq[cuintptr_t] {.nimcall, gcsafe, raises: [], tags: [], noinline.} =
      discard
    stackTraceOverrideGetDebuggingInfo: StackTraceOverrideGetDebuggingInfoProc = proc(
        programCounters: seq[cuintptr_t], maxLength: cint
    ): seq[StackTraceEntry] {.nimcall, gcsafe, raises: [], tags: [], noinline.} =
      discard
  ...
  proc registerStackTraceOverrideGetProgramCounters*(
      overrideProc: StackTraceOverrideGetProgramCountersProc
  ) =
    stackTraceOverrideGetProgramCounters = overrideProc

  proc registerStackTraceOverrideGetDebuggingInfo*(
      overrideProc: StackTraceOverrideGetDebuggingInfoProc
  ) =
  ...
  proc addStackTraceEntrySeq(
      result: var seq[StackTraceEntry], s: seq[StackTraceEntry]
  ) =
    for i in 0 ..< s.len:
      result.add(s[i])
  ...
  proc addDebuggingInfo*(s: seq[StackTraceEntry]): seq[StackTraceEntry] =
    ...
    for i in 0 ..< s.len:
      ...
      elif entry.procname.isNil and
          (entry.line == reraisedFromBegin or entry.line == reraisedFromEnd):
        result.addStackTraceEntrySeq(
          stackTraceOverrideGetDebuggingInfo(programCounters, maxStackTraceLines)
        )
        programCounters = @[]
        result.add(entry[])
        patchStackTraceEntry(result[result.high])
