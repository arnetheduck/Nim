...
type
  ...
  BaseChunk = object
    next: Chunk
    size: int
    head, tail: ptr ObjHeader
      # first and last object in chunk that
      # has a finalizer attached to it

const MaxSmallObject = 128
...
type
  ...
  SeqHeader = object
    # minor hack ahead: Since we know that seqs
...
var tlRegion {.threadvar.}: MemRegion
...
proc allocSlowPath(r: var MemRegion, size: int) =
  ...
  if (r.totalSize and 1) == 0:
    r.nextChunkSize =
      if r.totalSize < 64 * 1024:
        PageSize * nimMinHeapPages
      else:
        r.nextChunkSize * 2
...
proc setObstackPtr*(r: var MemRegion, sp: StackPtr) =
  ...
  if sp.current != nil and sp.current.next != nil:
    ...
    when false:
      ...
      for i in 0 .. high(r.freeLists):
        r.freeLists[i] = nil
      r.holes = nil
  if r.tail != nil:
    runFinalizers(r.tail, sp.bump)
...
proc newObj(typ: PNimType, size: int): pointer {.compilerRtl, raises: [].} =
  sysAssert typ.kind notin {tySequence, tyString},
    "newObj cannot be used to construct seqs"
...
proc newSeq(typ: PNimType, len: int): pointer {.compilerRtl, raises: [].} =
  let size =
    roundup(align(GenericSeqSize, typ.base.align) + len * typ.base.size, MemAlign)
...
proc growObj(
    regionUnused: var MemRegion, old: pointer, newsize: int
): pointer {.raises: [].} =
  let sh = cast[ptr SeqHeader](old -! sizeof(SeqHeader))
  let typ = sh.typ
  result = rawNewSeq(sh.region[], typ, roundup(newsize, MemAlign))
...
proc nimGCref(x: pointer) {.compilerproc.} =
  discard

proc nimGCunref(x: pointer) {.compilerproc.} =
  discard
