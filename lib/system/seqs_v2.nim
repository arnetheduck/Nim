...
proc newSeqPayload(cap, elemSize, elemAlign: int): pointer {.compilerRtl, raises: [].} =
  ...
  if cap > 0:
    var p = cast[ptr NimSeqPayloadBase](alignedAlloc0(
      align(sizeof(NimSeqPayloadBase), elemAlign) + cap * elemSize, elemAlign
    ))
    p.cap = cap
    result = p
  else:
...
proc newSeqPayloadUninit(
    cap, elemSize, elemAlign: int
): pointer {.compilerRtl, raises: [].} =
  # Used in `newSeqOfCap()`.
  if cap > 0:
    var p = cast[ptr NimSeqPayloadBase](alignedAlloc(
      align(sizeof(NimSeqPayloadBase), elemAlign) + cap * elemSize, elemAlign
    ))
    p.cap = cap
    result = p
...
): pointer {.noSideEffect, tags: [], raises: [], compilerRtl.} =
  {.noSideEffect.}:
    ...
    if addlen <= 0:
      ...
      if (p.cap and strlitFlag) == strlitFlag:
        q = cast[ptr NimSeqPayloadBase](alignedAlloc(
          headerSize + elemSize * newCap, elemAlign
        ))
...
proc zeroNewElements(
    len: int, q: pointer, addlen, elemSize, elemAlign: int
...
proc prepareSeqAddUninit(
  ...
  {.noSideEffect.}:
    ...
    if addlen <= 0:
      ...
      let newCap = max(resize(oldCap), len + addlen)
      if (p.cap and strlitFlag) == strlitFlag:
        var q = cast[ptr NimSeqPayloadBase](alignedAlloc(
          headerSize + elemSize * newCap, elemAlign
        ))
        copyMem(q +! headerSize, p +! headerSize, len * elemSize)
        q.cap = newCap
        ...
        let newSize = headerSize + elemSize * newCap
        var q =
          cast[ptr NimSeqPayloadBase](alignedRealloc(p, oldSize, newSize, elemAlign))
        q.cap = newCap
        result = q
...
proc grow*[T](x: var seq[T], newLen: Natural, value: T) {.nodestroy.} =
  ...
  if xu.p == nil or (xu.p.cap and not strlitFlag) < newLen:
    xu.p = cast[typeof(xu.p)](prepareSeqAddUninit(
      oldLen, xu.p, newLen - oldLen, sizeof(T), alignof(T)
    ))
  xu.len = newLen
...
proc add*[T](
    x: var seq[T], y: sink T
) {.magic: "AppendSeqElem", noSideEffect, nodestroy.} =
  ...
  {.cast(noSideEffect).}:
    ...
    if xu.p == nil or (xu.p.cap and not strlitFlag) < oldLen + 1:
      xu.p =
        cast[typeof(xu.p)](prepareSeqAddUninit(oldLen, xu.p, 1, sizeof(T), alignof(T)))
    xu.len = oldLen + 1
