...
const useGCC_builtins* =
  (defined(gcc) or defined(llvm_gcc) or defined(clang)) and useBuiltins
...
when useGCC_builtins:
  ...
  proc builtin_popcountll(
    x: culonglong
  ): cint {.importc: "__builtin_popcountll", cdecl.}

elif useVCC_builtins:
  # Counts the number of one bits (population count) in a 16-, 32-, or 64-byte unsigned integer.
  func builtin_popcnt16(
    a2: uint16
  ): uint16 {.importc: "__popcnt16", header: "<intrin.h>".}
  func builtin_popcnt32(
    a2: uint32
  ): uint32 {.importc: "__popcnt", header: "<intrin.h>".}
  func builtin_popcnt64(
    a2: uint64
  ): uint64 {.importc: "__popcnt64", header: "<intrin.h>".}
  ...
  func builtin_popcnt64(
    a: uint64
  ): cint {.importc: "_popcnt64", header: "<immintrin.h>".}
...
func countSetBitsImpl*(x: SomeInteger): int {.inline.} =
  ...
  when nimvm:
    ...
    when useGCC_builtins:
      ...
      when sizeof(x) <= 2:
        result = builtin_popcnt16(x.uint16).int
      elif sizeof(x) <= 4:
        result = builtin_popcnt32(x.uint32).int
      elif arch64:
        result = builtin_popcnt64(x.uint64).int
      else:
        result =
          builtin_popcnt32((x.uint64 and 0xFFFFFFFF'u64).uint32).int +
      ...
      when sizeof(x) <= 4:
        ...
        result =
          builtin_popcnt32((x.uint64 and 0xFFFFFFFF'u64).cint).int +
