...
const goNumSizeClasses = 67

type goMStats = object
...
proc goSetFinalizer(
  obj: pointer, f: pointer
) {.
  importc: "set_finalizer",
  codegenDecl: "$1 $2$3 __asm__ (\"main.Set_finalizer\");\n$1 $2$3",
  dynlib: goLib
.}

proc writebarrierptr(
  dest: PPointer, src: pointer
) {.
  importc: "writebarrierptr",
  codegenDecl: "$1 $2$3 __asm__ (\"main.Atomic_store_pointer\");\n$1 $2$3",
  dynlib: goLib
.}
...
proc GC_getStatistics(): string =
  ...
  result =
    "[GC] total allocated memory: " & $(mstats.total_alloc) & "\n" &
...
proc newSeq(typ: PNimType, len: int): pointer {.compilerproc.} =
  writebarrierptr(
    addr(result),
    newObj(typ, align(GenericSeqSize, typ.base.align) + len * typ.base.size),
  )
...
proc nimNewSeqOfCap(typ: PNimType, cap: int): pointer {.compilerproc.} =
  result =
    newObjNoInit(typ, align(GenericSeqSize, typ.base.align) + cap * typ.base.size)
...
proc typedMemMove(
  dest: pointer, src: pointer, size: uint
) {.importc: "typedmemmove", dynlib: goLib.}
...
proc deallocOsPages(r: var MemRegion) {.inline.} =
  discard

proc deallocOsPages() {.inline.} =
  discard
