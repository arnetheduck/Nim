...
proc boehmRegisterFinalizer(
  obj, ff, cd, off, ocd: pointer
) {.importc: "GC_register_finalizer", boehmGC.}
...
when not defined(useNimRtl):
  ...
  proc getOccupiedMem(): int =
    return boehmGetHeapSize() - boehmGetFreeBytes()

  proc getFreeMem(): int =
    return boehmGetFreeBytes()

  proc getTotalMem(): int =
    return boehmGetHeapSize()
...
proc boehmgc_finalizer(
    obj: pointer, typedFinalizer: (proc(x: pointer) {.cdecl, raises: [], gcsafe.})
) =
  typedFinalizer(obj)

proc newObj(typ: PNimType, size: int): pointer {.compilerproc.} =
  if ntfNoRefs in typ.flags:
    result = allocAtomic(size)
  else:
    result = alloc(size)
