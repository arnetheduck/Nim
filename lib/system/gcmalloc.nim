...
template chunkOverhead(): untyped = sizeof(Chunk)
...
iterator allObjects(m: var MemRegion): pointer {.inline.} =
  ...
  for s in elements(m.chunkStarts):
    ...
    if s in m.chunkStarts:
      let c = cast[PChunk](s shl MallocAlignShift)
      yield addr(c.data)
  m.locked = false
...
proc pageIndex(c: PChunk): int {.inline.} =
  result = cast[int](c) shr MallocAlignShift

proc pageAddr(p: pointer): PChunk {.inline.} =
  result = cast[PChunk](cast[uint](p) - cast[uint](chunkOverhead()))
