...
proc genericAssignAux(dest, src: pointer, n: ptr TNimNode, shallow: bool) {.benign.} =
  var
    d = cast[int](dest)
    s = cast[int](src)
  case n.kind
  of nkSlot:
    genericAssignAux(
      cast[pointer](d +% n.offset), cast[pointer](s +% n.offset), n.typ, shallow
    )
  of nkList:
    for i in 0 .. n.len - 1:
  ...
  of nkNone:
    sysAssert(false, "genericAssignAux")
  #else:
...
template deepSeqAssignImpl(operation, additionalArg) {.dirty.} =
  ...
  for i in 0 .. d.len - 1:
    operation(
      d.p +! (headerSize + i * bs), s.p +! (headerSize + i * bs), mt.base, additionalArg
    )
...
proc genericAssignAux(dest, src: pointer, mt: PNimType, shallow: bool) =
  ...
  of tyString:
    when defined(nimSeqsV2):
      ...
      if s2 == nil or shallow or (cast[PGenericSeq](s2).reserved and seqShallowFlag) != 0:
        unsureAsgnRef(x, s2)
      else:
  ...
  of tySequence:
    when defined(nimSeqsV2):
      ...
      if ntfNoRefs in mt.base.flags:
        ...
        copyMem(
          cast[pointer](dst +% align(GenericSeqSize, mt.base.align)),
...
proc objectInitAux(dest: pointer, n: ptr TNimNode) {.benign.} =
  ...
  case n.kind
  of nkNone:
    sysAssert(false, "objectInitAux")
  of nkSlot:
    objectInit(cast[pointer](d +% n.offset), n.typ)
  of nkList:
    for i in 0 .. n.len - 1:
...
proc genericResetAux(dest: pointer, n: ptr TNimNode) =
  ...
  case n.kind
  of nkNone:
    sysAssert(false, "genericResetAux")
  of nkSlot:
    genericReset(cast[pointer](d +% n.offset), n.typ)
  of nkList:
    for i in 0 .. n.len - 1:
      genericResetAux(dest, n.sons[i])
  of nkCase:
    var m = selectBranch(dest, n)
    if m != nil:
      genericResetAux(dest, m)
...
proc selectBranch(discVal, L: int, a: ptr array[0x7fff, ptr TNimNode]): ptr TNimNode =
  if discVal <% L:
    result = a[discVal]
    if result == nil:
      result = a[L]
...
proc FieldDiscriminantCheck(
    oldDiscVal, newDiscVal: int, a: ptr array[0x7fff, ptr TNimNode], L: int
) {.compilerproc.} =
