...
when sslVersion != "":
  ...
  when defined(osx):
    const versions =
      "(.3|.1.1|.38|.39|.41|.43|.44|.45|.46|.47|.48|.10|.1.0.2|.1.0.1|.1.0.0|.0.9.9|.0.9.8|)"
  else:
    const versions =
      "(.3|.1.1|.1.0.2|.1.0.1|.1.0.0|.0.9.9|.0.9.8|.48|.47|.46|.45|.44|.43|.41|.39|.38|.10|)"
...
type
  ...
  pem_password_cb* =
    proc(buf: cstring, size, rwflag: cint, userdata: pointer): cint {.cdecl.}
...
when useStaticLink:
  ...
  when defined(openssl10):
    ...
    proc OPENSSL_init_ssl*(
      opts: uint64, settings: uint8
    ): cint {.cdecl, dynlib: DLLSSLName, importc, discardable.}
  ...
  when defined(libressl) or defined(openssl10):
    ...
    proc SSL_CTX_set_ciphersuites*(
      ctx: SslCtx, str: cstring
  ...
  proc sslModule(): LibHandle {.inline, raises: [LibraryError], tags: [RootEffect].} =
    ...
    try:
      if sslMod.isNil:
        sslMod = loadLibPattern(DLLSSLName)
    except:
  ...
  proc utilModule(): LibHandle {.inline.} =
    ...
    if utilMod.isNil:
      utilMod = loadLibPattern(DLLUtilName)
  ...
  ): pointer {.raises: [LibraryError], tags: [RootEffect].} =
    sslModule().symNullable(name, alternativeName)

  proc sslSymThrows(
      name: string, alternativeName = ""
  ): pointer {.raises: [LibraryError].} =
  ...
  ): PSSL_METHOD {.raises: [LibraryError], tags: [RootEffect].} =
  ...
  proc CRYPTO_set_mem_functions(a, b, c: pointer) =
    let theProc = cast[proc(a, b, c: pointer) {.cdecl.}](utilModule().symNullable(
  ...
  proc ERR_load_BIO_strings*() =
    let theProc =
      cast[proc() {.cdecl.}](utilModule().symNullable("ERR_load_BIO_strings"))
    if not theProc.isNil:
      theProc()
  ...
  proc OpenSSL_add_all_algorithms*() =
    # TODO: Are we ignoring this on purpose? SSL GitHub CI fails otherwise.
    let theProc =
      cast[proc() {.cdecl.}](sslSymNullable("OPENSSL_add_all_algorithms_conf"))
    if not theProc.isNil:
      theProc()

  proc getOpenSSLVersion*(): culong =
    ## Return OpenSSL version as unsigned long or 0 if not available
    let theProc = cast[proc(): culong {.cdecl, gcsafe.}](utilSymNullable(
  ...
  proc SSL_in_init*(ssl: SslPtr): cint =
    ...
    let
      theProc {.global.} =
        cast[proc(ssl: SslPtr): cint {.cdecl, gcsafe.}](sslSymNullable(MainProc))
      # Fallback
      sslState {.global.} =
        cast[proc(ssl: SslPtr): cint {.cdecl, gcsafe.}](sslSymNullable("SSL_state"))
...
proc SSL_set_SSL_CTX*(
  ssl: SslPtr, ctx: SslCtx
): SslCtx {.cdecl, dynlib: DLLSSLName, importc.}

proc SSL_CTX_set_session_id_context*(
  context: SslCtx, sid_ctx: string, sid_ctx_len: int
) {.cdecl, dynlib: DLLSSLName, importc.}

proc SSL_get0_verified_chain*(
  ssl: SslPtr
): PSTACK {.cdecl, dynlib: DLLSSLName, importc.}

proc SSL_CTX_new*(meth: PSSL_METHOD): SslCtx {.cdecl, dynlib: DLLSSLName, importc.}
proc SSL_CTX_load_verify_locations*(
  ctx: SslCtx, CAfile: cstring, CApath: cstring
): cint {.cdecl, dynlib: DLLSSLName, importc.}

proc SSL_CTX_free*(arg0: SslCtx) {.cdecl, dynlib: DLLSSLName, importc.}
proc SSL_CTX_set_verify*(
  s: SslCtx, mode: int, cb: proc(a: int, b: pointer): int {.cdecl.}
) {.cdecl, dynlib: DLLSSLName, importc.}

proc SSL_get_verify_result*(ssl: SslPtr): int {.cdecl, dynlib: DLLSSLName, importc.}

proc SSL_CTX_set_cipher_list*(
  s: SslCtx, ciphers: cstring
): cint {.cdecl, dynlib: DLLSSLName, importc.}

proc SSL_CTX_use_certificate_file*(
  ctx: SslCtx, filename: cstring, typ: cint
): cint {.stdcall, dynlib: DLLSSLName, importc.}

proc SSL_CTX_use_certificate_chain_file*(
  ctx: SslCtx, filename: cstring
): cint {.stdcall, dynlib: DLLSSLName, importc.}

proc SSL_CTX_use_PrivateKey_file*(
  ctx: SslCtx, filename: cstring, typ: cint
): cint {.cdecl, dynlib: DLLSSLName, importc.}

proc SSL_CTX_check_private_key*(
  ctx: SslCtx
): cint {.cdecl, dynlib: DLLSSLName, importc.}

proc SSL_CTX_get_ex_new_index*(
  argl: clong, argp: pointer, new_func: pointer, dup_func: pointer, free_func: pointer
): cint {.cdecl, dynlib: DLLSSLName, importc.}

proc SSL_CTX_set_ex_data*(
  ssl: SslCtx, idx: cint, arg: pointer
): cint {.cdecl, dynlib: DLLSSLName, importc.}

proc SSL_CTX_get_ex_data*(
  ssl: SslCtx, idx: cint
): pointer {.cdecl, dynlib: DLLSSLName, importc.}

proc SSL_set_fd*(
  ssl: SslPtr, fd: SocketHandle
): cint {.cdecl, dynlib: DLLSSLName, importc.}

proc SSL_shutdown*(ssl: SslPtr): cint {.cdecl, dynlib: DLLSSLName, importc.}
proc SSL_set_shutdown*(
  ssl: SslPtr, mode: cint
) {.cdecl, dynlib: DLLSSLName, importc: "SSL_set_shutdown".}

proc SSL_get_shutdown*(
  ssl: SslPtr
): cint {.cdecl, dynlib: DLLSSLName, importc: "SSL_get_shutdown".}

proc SSL_connect*(ssl: SslPtr): cint {.cdecl, dynlib: DLLSSLName, importc.}
proc SSL_read*(
  ssl: SslPtr, buf: pointer, num: int
): cint {.cdecl, dynlib: DLLSSLName, importc.}

proc SSL_write*(
  ssl: SslPtr, buf: cstring, num: int
): cint {.cdecl, dynlib: DLLSSLName, importc.}

proc SSL_get_error*(
  s: SslPtr, ret_code: cint
): cint {.cdecl, dynlib: DLLSSLName, importc.}
...
proc BIO_new_mem_buf*(
  data: pointer, len: cint
): BIO {.cdecl, dynlib: DLLUtilName, importc.}

proc BIO_new_ssl_connect*(ctx: SslCtx): BIO {.cdecl, dynlib: DLLSSLName, importc.}
proc BIO_ctrl*(
  bio: BIO, cmd: cint, larg: int, arg: cstring
): int {.cdecl, dynlib: DLLUtilName, importc.}
...
proc BIO_read*(
  b: BIO, data: cstring, length: cint
...
proc ERR_error_string*(
  e: culong, buf: cstring
...
proc OPENSSL_sk_value*(
  stack: PSTACK, index: int
): pointer {.cdecl, dynlib: DLLSSLName, importc.}

proc d2i_X509*(
  px: ptr PX509, i: ptr ptr uint8, len: cint
): PX509 {.cdecl, dynlib: DLLUtilName, importc.}

proc i2d_X509*(
  cert: PX509, o: ptr ptr uint8
): cint {.cdecl, dynlib: DLLUtilName, importc.}
...
const useNimsAlloc = not defined(nimNoAllocForSSL) and not defined(gcDestructors)

when not useWinVersion and not defined(macosx) and not defined(android) and useNimsAlloc:
  proc allocWrapper(size: int): pointer {.cdecl.} =
    allocShared(size)

  proc reallocWrapper(p: pointer, newSize: int): pointer {.cdecl.} =
    if p == nil:
      if newSize > 0:
        result = allocShared(newSize)
    elif newSize == 0:
      deallocShared(p)
    else:
  ...
  proc CRYPTO_malloc_init*() =
    CRYPTO_set_mem_functions(
      cast[pointer](allocWrapper),
      cast[pointer](reallocWrapper),
      cast[pointer](deallocWrapper),
    )
...
proc SSL_CTX_ctrl*(
  ctx: SslCtx, cmd: cint, larg: clong, parg: pointer
): clong {.cdecl, dynlib: DLLSSLName, importc.}

proc SSL_CTX_callback_ctrl(
  ctx: SslCtx, typ: cint, fp: PFunction
): int {.cdecl, dynlib: DLLSSLName, importc.}

proc SSLCTXSetMode*(ctx: SslCtx, mode: int): int =
  result = SSL_CTX_ctrl(ctx, SSL_CTRL_MODE, clong mode, nil)

proc SSL_ctrl*(
  ssl: SslPtr, cmd: cint, larg: int, parg: pointer
): int {.cdecl, dynlib: DLLSSLName, importc.}
...
proc SSL_get_servername*(
  ssl: SslPtr, typ: cint = TLSEXT_NAMETYPE_host_name
): cstring {.cdecl, dynlib: DLLSSLName, importc.}
...
proc SSL_CTX_set_tlsext_servername_callback*(
    ctx: SslCtx, cb: proc(ssl: SslPtr, cb_id: int, arg: pointer): int {.cdecl.}
): int =
  ...
  result =
    SSL_CTX_callback_ctrl(ctx, SSL_CTRL_SET_TLSEXT_SERVERNAME_CB, cast[PFunction](cb))
...
type
  PskClientCallback* = proc(
    ssl: SslPtr,
    hint: cstring,
    identity: cstring,
    max_identity_len: cuint,
    psk: ptr uint8,
    max_psk_len: cuint,
  ): cuint {.cdecl.}

  PskServerCallback* = proc(
    ssl: SslPtr, identity: cstring, psk: ptr uint8, max_psk_len: cint
  ): cuint {.cdecl.}

proc SSL_CTX_set_psk_client_callback*(
  ctx: SslCtx, callback: PskClientCallback
) {.cdecl, dynlib: DLLSSLName, importc.}
  ## Set callback called when OpenSSL needs PSK (for client).

proc SSL_CTX_set_psk_server_callback*(
  ctx: SslCtx, callback: PskServerCallback
) {.cdecl, dynlib: DLLSSLName, importc.}
  ## Set callback called when OpenSSL needs PSK (for server).

proc SSL_CTX_use_psk_identity_hint*(
  ctx: SslCtx, hint: cstring
): cint {.cdecl, dynlib: DLLSSLName, importc.} ## Set PSK identity hint to use.
...
proc bioCtrlPending*(
  b: BIO
): cint {.cdecl, dynlib: DLLUtilName, importc: "BIO_ctrl_pending".}

proc bioRead*(
  b: BIO, Buf: cstring, length: cint
): cint {.cdecl, dynlib: DLLUtilName, importc: "BIO_read".}

proc bioWrite*(
  b: BIO, Buf: cstring, length: cint
): cint {.cdecl, dynlib: DLLUtilName, importc: "BIO_write".}

proc sslSetConnectState*(
  s: SslPtr
) {.cdecl, dynlib: DLLSSLName, importc: "SSL_set_connect_state".}

proc sslSetAcceptState*(
  s: SslPtr
) {.cdecl, dynlib: DLLSSLName, importc: "SSL_set_accept_state".}

proc sslRead*(
  ssl: SslPtr, buf: cstring, num: cint
): cint {.cdecl, dynlib: DLLSSLName, importc: "SSL_read".}

proc sslPeek*(
  ssl: SslPtr, buf: cstring, num: cint
): cint {.cdecl, dynlib: DLLSSLName, importc: "SSL_peek".}

proc sslWrite*(
  ssl: SslPtr, buf: cstring, num: cint
): cint {.cdecl, dynlib: DLLSSLName, importc: "SSL_write".}

proc sslSetBio*(
  ssl: SslPtr, rbio, wbio: BIO
) {.cdecl, dynlib: DLLSSLName, importc: "SSL_set_bio".}

proc sslDoHandshake*(
  ssl: SslPtr
): cint {.cdecl, dynlib: DLLSSLName, importc: "SSL_do_handshake".}
...
proc ErrRemoveState*(
  pid: cint
) {.cdecl, dynlib: DLLUtilName, importc: "ERR_remove_state".}

proc PEM_read_bio_RSA_PUBKEY*(
  bp: BIO, x: ptr PRSA, pw: pem_password_cb, u: pointer
): PRSA {.cdecl, dynlib: DLLUtilName, importc.}

proc PEM_read_RSA_PUBKEY*(
  fp: pointer, x: ptr PRSA, cb: pem_password_cb, u: pointer
): PRSA {.cdecl, dynlib: DLLUtilName, importc.}
...
proc PEM_read_RSAPrivateKey*(
  fp: pointer, x: ptr PRSA, cb: pem_password_cb, u: pointer
): PRSA {.cdecl, dynlib: DLLUtilName, importc.}

proc PEM_read_RSAPublicKey*(
  fp: pointer, x: ptr PRSA, cb: pem_password_cb, u: pointer
): PRSA {.cdecl, dynlib: DLLUtilName, importc.}

proc PEM_read_bio_RSAPublicKey*(
  bp: BIO, x: ptr PRSA, cb: pem_password_cb, u: pointer
): PRSA {.cdecl, dynlib: DLLUtilName, importc.}

proc PEM_read_bio_RSAPrivateKey*(
  bp: BIO, x: ptr PRSA, cb: pem_password_cb, u: pointer
): PRSA {.cdecl, dynlib: DLLUtilName, importc.}

proc RSA_private_encrypt*(
  flen: cint, fr: ptr uint8, to: ptr uint8, rsa: PRSA, padding: PaddingType
): cint {.cdecl, dynlib: DLLUtilName, importc.}

proc RSA_public_encrypt*(
  flen: cint, fr: ptr uint8, to: ptr uint8, rsa: PRSA, padding: PaddingType
): cint {.cdecl, dynlib: DLLUtilName, importc.}

proc RSA_private_decrypt*(
  flen: cint, fr: ptr uint8, to: ptr uint8, rsa: PRSA, padding: PaddingType
): cint {.cdecl, dynlib: DLLUtilName, importc.}

proc RSA_public_decrypt*(
  flen: cint, fr: ptr uint8, to: ptr uint8, rsa: PRSA, padding: PaddingType
): cint {.cdecl, dynlib: DLLUtilName, importc.}
...
# RSA key functions
proc PEM_read_bio_PrivateKey*(
  bp: BIO, x: ptr EVP_PKEY, cb: pointer, u: pointer
): EVP_PKEY {.lcrypto.}

proc EVP_PKEY_free*(p: EVP_PKEY) {.lcrypto.}
proc EVP_DigestSignInit*(
  ctx: EVP_MD_CTX, pctx: ptr EVP_PKEY_CTX, typ: EVP_MD, e: ENGINE, pkey: EVP_PKEY
): cint {.lcrypto.}

proc EVP_DigestInit_ex*(
  ctx: EVP_MD_CTX, typ: EVP_MD, engine: SslPtr = nil
): cint {.lcrypto.}

proc EVP_DigestUpdate*(ctx: EVP_MD_CTX, data: pointer, len: cuint): cint {.lcrypto.}
proc EVP_DigestFinal_ex*(
  ctx: EVP_MD_CTX, buffer: pointer, size: ptr cuint
): cint {.lcrypto.}

proc EVP_DigestSignFinal*(
  ctx: EVP_MD_CTX, data: pointer, len: ptr csize_t
): cint {.lcrypto.}
...
when defined(macosx) or defined(windows):
  ...
  proc EVP_MD_CTX_create*(): EVP_MD_CTX {.
    cdecl, importc: "EVP_MD_CTX_new", dynlib: DLLUtilName
  .}

  proc EVP_MD_CTX_destroy*(
    ctx: EVP_MD_CTX
  ) {.cdecl, importc: "EVP_MD_CTX_free", dynlib: DLLUtilName.}

  proc EVP_MD_CTX_cleanup*(
    ctx: EVP_MD_CTX
  ): cint {.cdecl, importc: "EVP_MD_CTX_cleanup", dynlib: DLLUtilName.}
...
proc md5_Update*(
  c: var MD5_CTX, data: pointer, len: csize_t
): cint {.importc: "MD5_Update".}
...
when not defined(nimDisableCertificateValidation) and not defined(windows):
  ...
  when useOpenssl3:
    proc SSL_get1_peer_certificate*(
      ssl: SslCtx
    ): PX509 {.cdecl, dynlib: DLLSSLName, importc.}

    proc SSL_get_peer_certificate*(ssl: SslCtx): PX509 =
      SSL_get1_peer_certificate(ssl)

  elif useStaticLink:
    proc SSL_get_peer_certificate*(
      ssl: SslCtx
    ): PX509 {.cdecl, dynlib: DLLSSLName, importc.}
    ...
    proc SSL_get_peer_certificate*(ssl: SslCtx): PX509 =
      let methodSym =
        sslSymNullable("SSL_get_peer_certificate", "SSL_get1_peer_certificate")
      if methodSym.isNil:
        raise newException(
          LibraryError,
          "Could not load SSL_get_peer_certificate or SSL_get1_peer_certificate",
        )
      let method2Proc =
        cast[proc(ssl: SslCtx): PX509 {.cdecl, gcsafe, raises: [].}](methodSym)
      return method2Proc(ssl)

  proc X509_get_subject_name*(
    a: PX509
  ): PX509_NAME {.cdecl, dynlib: DLLSSLName, importc.}

  proc X509_get_issuer_name*(
    a: PX509
  ): PX509_NAME {.cdecl, dynlib: DLLUtilName, importc.}

  proc X509_NAME_oneline*(
    a: PX509_NAME, buf: cstring, size: cint
  ): cstring {.cdecl, dynlib: DLLSSLName, importc.}

  proc X509_NAME_get_text_by_NID*(
    subject: cstring, NID: cint, buf: cstring, size: cint
  ): cint {.cdecl, dynlib: DLLSSLName, importc.}

  proc X509_check_host*(
    cert: PX509, name: cstring, namelen: cint, flags: cuint, peername: cstring
  ): cint {.cdecl, dynlib: DLLSSLName, importc.}
...
proc SSL_CTX_set_alpn_protos*(
  ctx: SslCtx, protos: cstring, protos_len: cuint
): cint {.cdecl, dynlib: DLLSSLName, importc.}

proc SSL_set_alpn_protos*(
  ssl: SslPtr, protos: cstring, protos_len: cuint
): cint {.cdecl, dynlib: DLLSSLName, importc.}

proc SSL_CTX_set_alpn_select_cb*(
  ctx: SslCtx,
  cb: proc(
    ssl: SslPtr,
...
proc SSL_get0_alpn_selected*(
  ssl: SslPtr, data: ptr cstring, len: ptr cuint
) {.cdecl, dynlib: DLLSSLName, importc.}

proc SSL_CTX_set_next_protos_advertised_cb*(
  ctx: SslCtx,
...
proc SSL_CTX_set_next_proto_select_cb*(
  ctx: SslCtx,
  cb: proc(
    s: SslPtr,
    out_proto: cstring,
...
proc SSL_get0_next_proto_negotiated*(
  s: SslPtr, data: ptr cstring, len: ptr cuint
) {.cdecl, dynlib: DLLSSLName, importc.}
