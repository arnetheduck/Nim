...
proc cleanupTemp(p: BProc, returnType: PType, tmp: TLoc): bool =
  if returnType.kind in {tyVar, tyLent}:
    ...
    result = false
  elif hasDestructor(returnType) and
      getAttachedOp(p.module.g.graph, returnType, attachedDestructor) != nil:
...
proc genArg(
  ...
  if n.kind == nkStringToCString:
    ...
    let needsIndirect =
      mapType(p.config, n[0].typ, mapTypeChooser(n[0]) == skParam) != ctArray
    if needsIndirect:
    ...
    if callee.kind == nkSym and
        ...
        {lfHeader, lfNoDecl} * callee.sym.loc.flags != {} and needsIndirect:
    ...
    if param.typ.kind in {tyVar, tyPtr, tyRef, tySink}:
      ...
      if not sameBackendTypePickyAliases(typ, n.typ.skipTypes(abstractPtrs)):
        a.snippet = "(($1) ($2))" % [getTypeDesc(p.module, param.typ), rdCharLoc(a)]
