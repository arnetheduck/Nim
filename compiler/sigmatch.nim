...
import
  ast, astalgo, semdata, types, msgs, renderer, lookups, semtypinst, magicsys, idents,
  lexer, options, parampatterns, trees, linter, lineinfos, lowerings, modulegraphs,
  concepts, layeredtable
...
type
  ...
  TCandidate* = object
    ...
    matchedErrorType*: bool
      # match is considered successful after matching
...
proc markUsed*(
  c: PContext, info: TLineInfo, s: PSym, checkStyle = true, isGenericInstance = false
)

proc markOwnerModuleAsUsed*(c: PContext, s: PSym)

proc initCandidateAux(ctx: PContext, callee: PType): TCandidate {.inline.} =
  result = TCandidate(
    c: ctx,
    exactMatches: 0,
    subtypeMatches: 0,
...
proc matchGenericParam(m: var TCandidate, formal: PType, n: PNode) =
  ...
  while formalBase.kind == tyGenericParam and formalBase.genericParamHasConstraints:
  ...
  if binding != nil and callee.kind in routineKinds:
    ...
    if genericMatch != csNoMatch:
      ...
      if genericMatch == csMatch: # csEmpty if not fully instantiated
        ...
        let typ =
          ctx.instantiateOnlyProcType(ctx, result.bindings, callee, binding.info)
        result.callee = typ
      else:
...
proc sumGeneric(t: PType): int =
  ...
  while true:
    case t.kind
    of tyAlias, tySink, tyNot:
      t = t.skipModifier
    of tyArray, tyRef, tyPtr, tyDistinct, tyUncheckedArray, tyOpenArray, tyVarargs,
        tySet, tyRange, tySequence, tyLent, tyOwned, tyVar:
      t = t.elementType
      inc result
    of tyBool,
        tyChar,
...
proc concreteType(c: TCandidate, t: PType, f: PType = nil): PType =
  ...
  of tyOwned:
    ...
    if f != nil and f.hasElementType and
        f.elementType.skipTypes({tyBuiltInTypeClass, tyOr}).kind == tyProc:
  ...
  if a.kind == f.kind:
    ...
    if k == f.kind:
      ...
      if isIntLit(ab):
        ...
        result = isSubrange
    elif a.kind == tyInt and f.kind in {tyRange, tyInt .. tyInt64, tyUInt .. tyUInt64} and
...
proc procTypeRel(c: var TCandidate, f, a: PType): TTypeRelation =
  ...
  of tyProc:
    ...
    if f.signatureLen != a.signatureLen:
      return
    result = isEqual
...
template subtypeCheck() =
  ...
  of isConvertible:
    if f.last.skipTypes(abstractInst).kind != tyOpenArray:
      ...
      result = isNone
  of isSubtype:
    if f.last.skipTypes(abstractInst).kind in {tyRef, tyPtr, tyVar, tyLent, tyOwned}:
...
proc enterConceptMatch(
    c: var TCandidate, f, a: PType, flags: TTypeRelFlags
): TTypeRelation =
  var
  ...
  let mres =
    concepts.conceptMatch(c.c, concpt, a, c.bindings, container, flags = conceptFlags)
  if mres: isGeneric else: isNone
...
proc typeRel(
  ...
  of tyTuple:
    if a.kind == tyTuple:
      result = recordRel(c, f, a, flags)
  of tyObject:
    let effectiveArgType =
      if useTypeLoweringRuleInTypeClass:
        a
      else:
        reduceToBase(a)
  ...
  of tyGenericParam:
    ...
    if x == nil:
      ...
      let inheritancePenaltyOld = c.inheritancePenalty
        ...
        if c.c.inGenericContext > 0 and aOrig.n == nil and not c.isNoCall:
          ...
          if result != isNone and f.n != nil:
            ...
            if not exprStructuralEquivalent(r, aOrig.n) and
                not (
                  aOrig.n != nil and aOrig.n.kind == nkIntLit and
...
proc convertLiteral(
    kind: TNodeKind, c: PContext, m: TCandidate, n: PNode, newType: PType
): PNode =
  ...
  of nkCharLit .. nkUInt64Lit:
    if n.kind != nkUInt64Lit and not sameTypeOrNil(n.typ, newType) and
        isOrdinalType(newType):
      let value = n.intVal
  ...
  of nkFloatLit .. nkFloat64Lit:
    if newType.skipTypes(abstractVarRange - {tyTypeDesc}).kind == tyFloat:
      ...
      return
  of nkSym:
    if n.sym.kind == skEnumField and not sameTypeOrNil(n.sym.typ, newType) and
        isOrdinalType(newType):
...
proc paramTypesMatchAux(
  ...
  block instantiateGenericRoutine:
    ...
    while r in {isBothMetaConvertible, isInferred, isInferredConvertible} and
        lastBindingCount != m.bindings.currentLen and instantiationCounter < 100:
  ...
  of isGeneric:
    ...
    if arg.typ == nil:
      result = arg
        ...
        cmpInheritancePenalty(oldInheritancePenalty, m.inheritancePenalty) > 0:
      result = implicitConv(nkHiddenSubConv, f, arg, m, c)
...
proc arrayConstr(c: PContext, n: PNode): PType =
  ...
  addSonSkipIntLit(result, skipTypes(n.typ, {tyVar, tyLent, tyOrdinal}), c.idgen)
    ...
    if not containsOrIncl(marker, formal.position):
      if formal.ast == nil:
        ...
        if formal.ast.kind == nkEmpty:
          ...
          typeMismatch(
            c.config, formal.ast.info, formal.typ, formal.ast.typ, formal.ast
          )
