...
import
  std/
    [os, strutils, strtabs, algorithm, json, osproc, tables, intsets, xmltree, sequtils]
...
proc toLangSymbol(k: TSymKind, n: PNode, baseName: string): LangSymbol =
  ...
  result = LangSymbol(name: baseName.nimIdentNormalize, symKind: k.toHumanStr)
...
proc genItem(
  ...
  var r: TSrcGen = initTokRender(
    n,
    {
      ...
      renderNoProcDefs, renderExpandUsing, renderNoPostfix,
    },
  )
  while true:
    getNextTok(r, kind, literal)
  ...
  if k in routineKinds and symNameNode.kind == nkSym:
    ...
    for e in etyp.n:
      ...
      setIndexTerm(
        ...
        id = symbolOrId,
        term = plain,
        linkTitle = symNameNode.sym.name.s & '.' & plain,
...
proc genJsonItem(
  ...
  var
    ...
    r: TSrcGen
    renderFlags = {
      renderNoBody, renderNoComments, renderDocComments, renderExpandUsing,
      renderNoPostfix,
    }
  if nonExports:
    renderFlags.incl renderNonExportedFields
  ...
  result = JsonItem(
    json: %{"name": %name, "type": %($k), "line": %n.info.line.int, "col": %n.info.col}
  )
  if comm != nil:
    result.rst = comm
...
proc setIndexTitle(d: PDoc, useMetaTitle: bool) =
  ...
  if useMetaTitle and d.meta[metaTitle].len != 0:
    ...
    term =
      if d.standaloneDoc:
        filename # keep .rst/.md extension
      else:
        changeFileExt(filename, "") # rm .nim extension
    linkTitle =
      if d.standaloneDoc:
        term # keep .rst/.md extension
      else:
...
proc genSection(d: PDoc, kind: TSymKind, groupedToc = false) =
  ...
  let sectionValues = @[
    "sectionID", $ord(kind), "sectionTitleID", $(ord(kind) + 50), "sectionTitle", title
  ]
