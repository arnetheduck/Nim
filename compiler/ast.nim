...
import lineinfos, options, ropes, idents, int128, wordrecg
...
type
  TSymFlag* = enum # 63 flags!
    ...
    sfDirty
      # template is not hygienic (old styled template) module,
...
const
  ...
  sfCppMember* = {sfVirtual, sfMember, sfConstructor}
    # proc is a C++ member, meaning it will be attached to the type definition
...
type TTypeKind* = enum
  ...
  tyError
    # used as erroneous type (for idetools)
...
const
  ...
  tyTypeClasses* = {
    ...
    tyConcept, tyAnd, tyOr, tyNot, tyAnything,
  }
...
type
  ...
  TNodeFlag* = enum
    ...
    nfDisabledOpenSym
      # temporary: node should be nkOpenSym but cannot
  ...
  TTypeFlag* = enum # keep below 32 for efficiency reasons (now: 47)
    ...
    tfRequiresInit
      # type contains a "not nil" constraint somewhere or
...
var eqTypeFlags* =
  {tfIterator, tfNotNil, tfVarIsPtr, tfGcSafe, tfNoSideEffect, tfIsOutParam}
...
const
  ...
  ctfeWhitelist* = {
    ...
    mUnaryMinusF64, mCharToStr, mBoolToStr, mCStrToStr, mStrToStr, mEnumToStr, mAnd,
    mOr, mEqStr, mLeStr, mLtStr, mEqSet, mLeSet, mLtSet, mMulSet, mPlusSet, mMinusSet,
    mXorSet, mConStrStr, mAppendStrCh, mAppendStrStr, mAppendSeqElem, mInSet, mRepr,
    mOpenArrayToSeq,
  }
...
const
  ...
  PersistentNodeFlags*: TNodeFlags = {
    ...
    nfFirstWrite, nfSkipFieldChecking, nfDisabledOpenSym,
  }
  namePos* = 0
  ...
  nkIdentKinds* =
    {nkIdent, nkSym, nkAccQuoted, nkOpenSymChoice, nkClosedSymChoice, nkOpenSym}
...
proc genericParamHasConstraints*(t: PType): bool {.inline.} =
  t.sons.len > 0

proc hasElementType*(t: PType): bool {.inline.} =
  t.sons.len > 0

proc isEmptyTupleType*(t: PType): bool {.inline.} =
  t.sons.len == 0

proc isSingletonTupleType*(t: PType): bool {.inline.} =
  t.sons.len == 1

proc genericConstraint*(t: PType): PType {.inline.} =
  t.sons[0]
...
iterator paramTypes*(t: PType): (int, PType) =
  for i in FirstParamAt ..< t.sons.len:
    yield (i, t.sons[i])

iterator paramTypePairs*(a, b: PType): (PType, PType) =
  for i in FirstParamAt ..< a.sons.len:
    yield (a.sons[i], b.sons[i])
...
proc newType*(
    kind: TTypeKind, idgen: IdGenerator, owner: PSym, son: sink PType = nil
): PType =
...
proc propagateToOwner*(owner, elem: PType, propagateHasAsgn = true) =
  ...
  if mask != {} and propagateHasAsgn:
    ...
    if o2.kind in {tyTuple, tyObject, tyArray, tySequence, tyString, tySet, tyDistinct}:
      o2.flags.incl mask
...
proc toObject*(typ: PType): PType =
  ...
  if t.kind == tyRef: t.elementType else: typ
