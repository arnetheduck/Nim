...
proc typeAllowedAux(
  ...
  of tyVar, tyLent:
    if kind in {skProc, skFunc, skConst} and (views notin c.features):
      ...
      ((kind != skResult or taObjField in flags) and views notin c.features) or
      (kind == skParam and {taIsCastable, taObjField, taTupField} * flags == {})
    ):
      ...
      of tyVar, tyLent:
        if taHeap notin flags:
          result = t2 # ``var var`` is illegal on the heap
      of tyOpenArray:
  ...
  of tyIterable:
    if kind notin {skParam} or taNoUntyped in flags:
      result = t # tyIterable is only for templates and macros.
  of tyStatic:
    if kind notin {skParam}:
      result = t
  of tyVoid:
    if taVoid notin flags:
      result = t
  of tyTypeClasses:
  ...
  of tySink:
    ...
    if kind != skParam or taIsOpenArray in flags or
        t.elementType.kind in {tySink, tyLent, tyVar}:
      result = t
    else:
  ...
  of tyArray:
    if t.elementType.kind == tyTypeDesc:
      ...
      result = t.elementType
  of tyRef:
    if kind == skConst and taIsDefaultField notin flags:
      result = t
    else:
      result = typeAllowedAux(marker, t.elementType, kind, c, flags + {taHeap})
  of tyPtr:
  ...
  of tySet:
    result = typeAllowedAux(marker, t.elementType, kind, c, flags)
  of tyObject:
    if kind in {skProc, skFunc, skConst} and t.baseClass != nil and
        taIsDefaultField notin flags:
      result = t
    else:
  ...
  of tyError:
    ...
    result = nil
  of tyOwned:
    if t.hasElementType and
        t.skipModifier.skipTypes(abstractInst).kind in {tyRef, tyPtr, tyProc}:
