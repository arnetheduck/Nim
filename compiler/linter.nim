...
import
  options, ast, msgs, idents, lineinfos, wordrecg, astmsgs, semdata, packages,
  modulegraphs
export packages
...
template styleCheckDef*(ctx: PContext, info: TLineInfo, sym: PSym, k: TSymKind) =
  ...
  ctx.config.belongsToProjectPackageMaybeNil(getModule(ctx.graph, info.fileIndex)) and
    # ignore foreign packages
...
template checkPragmaUse*(
  ...
  if {optStyleHint, optStyleError} * ctx.config.globalOptions != {} and
    # ignore if styleChecks are off
  hintName in ctx.config.notes and # ignore if name checks are not requested
  ctx.config.belongsToProjectPackageMaybeNil(getModule(ctx.graph, info.fileIndex)):
    # ignore foreign packages
