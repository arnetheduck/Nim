...
proc annotateClosureConv(n: PNode) =
  ...
  of {nkNone .. nkNilLit}:
    discard
  of nkTupleConstr:
    if n.typ.kind == tyProc and n.typ.callConv == ccClosure and n[0].typ.kind == tyProc and
        n[0].typ.callConv != ccClosure:
      # restores `transf.generateThunk`
      n[0] =
        newTreeIT(nkHiddenSubConv, n[0].info, n.typ, newNodeI(nkEmpty, n[0].info), n[0])
...
proc fitDefaultNode(c: PContext, n: var PNode, expectedType: PType) =
  ...
  if expectedType != nil and oldType != expectedType:
    ...
    changeType(c, n, expectedType, true)
      # infer types for default fields value
  ...
  if n.kind != nkNilLit:
    typeAllowedCheck(
      c, n.info, n.typ, skConst, {taProcContextIsNotMacro, taIsDefaultField}
    )
...
proc semArrayIndexConst(c: PContext, e: PNode, info: TLineInfo): PType =
  ...
  if x.kind in {nkIntLit .. nkUInt64Lit}:
    result = makeRangeType(c, 0, x.intVal - 1, info, x.typ.skipTypes({tyTypeDesc}))
  else:
  ...
  if isRange(n):
    ...
    if e.typ.kind == tyFromExpr:
      ...
      if e.intVal < 0:
        if e.kind in {nkIntLit .. nkInt64Lit}:
          ...
          localError(
            c.config,
            n.info,
            "Array length can't exceed its maximum value (9223372036854775807), but was " &
              $cast[BiggestUInt](e.intVal),
          )
      ...
      if e.typ.kind == tyStatic:
        ...
        if e.typ.skipModifier.kind != tyGenericParam and
            not isOrdinalType(e.typ.skipModifier):
          let info =
            if n.safeLen > 1:
              n[1].info
            else:
...
proc semArray(c: PContext, n: PNode, prev: PType): PType =
  ...
  if n.len == 3:
    ...
    if indxB.kind notin {tyGenericParam, tyStatic, tyFromExpr} and
      ...
      elif indxB.kind != tyRange and lengthOrd(c.config, indxB) > high(uint16).int:
...
proc semIdentWithPragma(
    c: PContext, kind: TSymKind, n: PNode, allowed: TSymFlags, fromTopLevel = false
): PSym =
  if n.kind == nkPragmaExpr:
    ...
    let invalidPragmasForPush =
      if fromTopLevel and sfWasGenSym notin result.flags:
        {}
      else:
        {wExportc, wExportCpp, wDynlib}
    case kind
    of skField:
      implicitPragmas(c, result, n.info, fieldPragmas)
    of skVar:
      implicitPragmas(c, result, n.info, varPragmas - invalidPragmasForPush)
    of skLet:
      implicitPragmas(c, result, n.info, letPragmas - invalidPragmasForPush)
    of skConst:
      implicitPragmas(c, result, n.info, constPragmas - invalidPragmasForPush)
    else:
      discard
...
proc semCaseBranchSetElem(c: PContext, n, b: PNode, covered: var Int128): PNode =
  if isRange(b):
    checkSonsLen(b, 3, c.config)
...
proc semCaseBranch(
    c: PContext, n, branch: PNode, branchIndex: int, covered: var Int128
) =
  ...
  of nkRecWhen:
    ...
    for i in 0 ..< a.len:
      ...
      of nkElse:
        ...
        if branch == nil and not cannotResolve:
          branch = it[0]
        idx = 0
      else:
        illFormedAst(n, c.config)
...
proc semAnyRef(c: PContext, n: PNode, kind: TTypeKind, prev: PType): PType =
  if n.len < 1:
    ...
    if tfPartial in result.flags:
      if result.elementType.kind == tyObject:
        incl(result.elementType.flags, tfPartial)
    ...
    of tySink:
      ...
      discard
    if result.kind == tyRef and c.config.selectedGC in {gcArc, gcOrc, gcAtomicArc} and
...
proc liftParamType(
  ...
  of tyStatic:
    ...
    result = addImplicitGeneric(
      c, newTypeS(tyStatic, c, base), paramTypId, info, genericParams, paramName
    )
    if result != nil:
  ...
  of tySequence, tySet, tyArray, tyOpenArray, tyVar, tyLent, tyPtr, tyRef, tyProc,
    ...
    if paramType.kind == tySequence and paramType.elementType.kind == tyNone:
      let typ = newTypeS(tyBuiltInTypeClass, c, newTypeS(paramType.kind, c))
  ...
  of tyGenericInst:
    ...
    if paramType.isConcept:
      return
        addImplicitGeneric(c, paramType, paramTypId, info, genericParams, paramName)
    else:
  ...
  of tyUserTypeClasses, tyBuiltInTypeClass, tyCompositeTypeClass, tyAnd, tyOr, tyNot,
    ...
    result = addImplicitGeneric(
      c,
      copyType(paramType, c.idgen, getCurrOwner(c)),
      paramTypId,
      info,
      genericParams,
      paramName,
...
proc containsGenericInvocationWithForward(n: PNode): bool =
  if n.kind == nkSym and n.sym.ast != nil and n.sym.ast.len > 1 and
      n.sym.ast[2].kind == nkObjectTy:
    for p in n.sym.ast[2][^1]:
      if p.kind == nkIdentDefs and p[1].typ != nil and
          p[1].typ.kind == tyGenericInvocation and p[1][0].kind == nkSym and
          p[1][0].typ.kind == tyForward:
        return true
  return false
...
proc semTypeOf(c: PContext, n: PNode, prev: PType): PType =
  ...
  defer:
    ...
    # compiles can raise an exception
...
proc semTypeNode(c: PContext, n: PNode, prev: PType): PType =
  ...
  of nkBracketExpr:
    ...
    of mBuiltinType:
      case s.name.s
      of "lent":
        result = semAnyRef(c, n, tyLent, prev)
      of "sink":
        result = semAnyRef(c, n, tySink, prev)
      of "owned":
  ...
  of nkIdent, nkAccQuoted:
    ...
    if s.typ == nil:
      ...
      if alias != nil:
        ...
        if s.typ.kind notin {
          tyBool, tyChar, tyInt .. tyInt64, tyFloat .. tyFloat128, tyUInt .. tyUInt64
        }:
  ...
  of nkProcTy, nkIteratorTy:
    if n.len == 0 or n[0].kind == nkEmpty:
      ...
      if n.kind == nkIteratorTy and result.kind == tyProc:
        result.flags.incl(tfIterator)
      if result.callConv == ccClosure and
