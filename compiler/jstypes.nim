...
proc genObjectInfo(p: PProc, typ: PType, name: Rope) =
  ...
  if (typ.kind == tyObject) and (typ.baseClass != nil):
    p.g.typeInfo.addf(
      "$1.base = $2;$n", [name, genTypeInfo(p, typ.baseClass.skipTypes(skipPtrs))]
    )
...
proc genEnumInfo(p: PProc, typ: PType, name: Rope) =
  ...
  if typ.baseClass != nil:
    p.g.typeInfo.addf("$1.base = $2;$n", [name, genTypeInfo(p, typ.baseClass)])

proc genTypeInfo(p: PProc, typ: PType): Rope =
  let t = typ.skipTypes(
    {tyGenericInst, tyDistinct, tyAlias, tySink, tyOwned} + tyUserTypeClasses
  )
  ...
  of tyVar, tyLent, tyRef, tyPtr, tySequence, tyRange, tySet, tyOpenArray:
    var s =
      ...
      [result, rope(ord(t.kind))]
    prepend(p.g.typeInfo, s)
    p.g.typeInfo.addf("$1.base = $2;$n", [result, genTypeInfo(p, t.elementType)])
  of tyArray:
    var s =
      "var $1 = {size: 0, kind: $2, base: null, node: null, finalizer: null};$n" %
      [result, rope(ord(t.kind))]
  ...
  of tyStatic:
    if t.n != nil:
      result = genTypeInfo(p, skipModifier t)
    else:
