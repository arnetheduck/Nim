...
proc genStringLiteralDataOnlyV1(m: BModule, s: string, result: var Rope) =
  ...
  result.add tmp
  m.s[cfsStrData].addf(
    "STRING_LITERAL($1, $2, $3);$n", [tmp, makeCString(s), rope(s.len)]
  )
...
proc genStringLiteralV1(m: BModule, n: PNode, result: var Rope) =
  if s.isNil:
    ...
    if id == m.labels:
      ...
      appcg(m, result, "((#NimStringDesc*) &$1$2)", [m.tmpBase, id])

# ------ Version 2: destructor based strings and seqs -----------------------

proc genStringLiteralDataOnlyV2(m: BModule, s: string, result: Rope, isConst: bool) =
  m.s[cfsStrData].addf(
    "static $4 struct {$n" & "  NI cap; NIM_CHAR data[$2+1];$n" &
      "} $1 = { $2 | NIM_STRLIT_FLAG, $3 };$n",
    [result, rope(s.len), makeCString(s), rope(if isConst: "const" else: "")],
  )
...
proc genStringLiteralV2(m: BModule, n: PNode, isConst: bool, result: var Rope) =
  ...
  if id == m.labels:
    ...
    # string literal not found in the cache:
    m.s[cfsStrData].addf(
      "static $4 NimStringV2 $1 = {$2, (NimStrPayload*)&$3};$n",
...
proc genStringLiteralDataOnly(
    m: BModule, s: string, info: TLineInfo, isConst: bool, result: var Rope
) =
...
proc genStringLiteral(m: BModule, n: PNode, result: var Rope) =
  case detectStrVersion(m)
  of 0, 1:
    genStringLiteralV1(m, n, result)
  of 2:
    genStringLiteralV2(m, n, isConst = true, result)
  else:
    localError(
