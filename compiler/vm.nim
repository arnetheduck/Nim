  ...
  while true:
    ...
    of opcWrDeref:
      ...
      of rkNodeAddr:
        ...
        if nAddr[] == nil:
          stackTrace(c, tos, pc, "opcWrDeref internal error") # refs bug #16613
    ...
    of opcSymIsInstantiationOf:
      ...
      if a.kind == nkSym and a.sym.kind in skProcKinds and b.kind == nkSym and
        ...
        regs[ra].intVal =
          if sfFromGeneric in a.sym.flags and a.sym.instantiatedFrom == b.sym: 1 else: 0
      else:
      ...
      if prc.offset < -1:
        ...
        if c.callDepth <= 0:
          if allowInfiniteRecursion in c.features:
            ...
            stackTraceAux(c, tos, pc)
            globalError(
              c.config, c.debug[pc], errCallDepthExceeded % $c.config.maxCallDepthVM
            )
        dec(c.callDepth)
        tos = newFrame
        updateRegsAlias
    ...
    of opcLdGlobalDerefFFI:
      ...
      if typ.kind == tyPtr:
        ...
        stackTrace(
          ...
          tos,
          pc,
          "opcLdDeref unsupported type: " & $(typeToString(typ), typ.elementType.kind),
        )
    ...
    of opcNGetType:
      ...
      of 3:
        ...
        ensureKind(rkNode)
        if regs[rb].kind == rkNode and regs[rb].node.typ != nil:
          regs[ra].node = opMapTypeInstToAst(
            c.cache, regs[rb].node.typ, c.debug[pc], c.idgen, skipAlias = true
          )
        elif regs[rb].kind == rkNode and regs[rb].node.kind == nkSym and
            regs[rb].node.sym.typ != nil:
          regs[ra].node = opMapTypeInstToAst(
            c.cache, regs[rb].node.sym.typ, c.debug[pc], c.idgen, skipAlias = true
          )
        else:
          stackTrace(c, tos, pc, "node has no type")
    of opcNGetSize:
    ...
    of opcTypeTrait:
      ...
      while typ.kind == tyTypeDesc and typ.hasElementType:
        typ = typ.skipModifier
      createStr regs[ra]
  ...
  if sym.kind in routineKinds:
    if sym.typ.paramsLen != args.len:
      ...
      if not isEmptyType(sym.typ.returnType) or sym.kind == skMacro:
        putIntoReg(
          tos.slots[0], getNullValue(c, sym.typ.returnType, sym.info, c.config)
        )
