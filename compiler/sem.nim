...
proc fixupTypeAfterEval(
    c: PContext, evaluated, eOrig: PNode, producedClosure: var bool
): PNode =
...
const errMissingGenericParamsForTemplate = "'$1' has unspecified generic parameters"
...
proc semMacroExpr(
  ...
  if c.config.macrosToExpand.hasKey(sym.name.s):
    message(
      ...
      nOrig.info,
      hintExpandMacro,
      renderTree(result, {renderNonExportedFields, renderDocComments, renderNoComments}),
    )
    ...
    if child != nil:
      ...
      result = semExpr(
        ...
        newTree(
          ...
          newTree(
            nkBracketExpr,
            newSymNode(getSysSym(c.graph, a.info, "arrayWithDefault"), a.info),
            typeNode,
          ),
          node,
        ),
