...
proc processNote(config: ConfigRef, n: PNode) =
  ...
  of wHint:
    handleNote(hintMin .. hintMax, config.notes)
  of wWarning:
    handleNote(warnMin .. warnMax, config.notes)
  of wWarningAsError:
    handleNote(warnMin .. warnMax, config.warningAsErrors)
  of wHintAsError:
    handleNote(hintMin .. hintMax, config.warningAsErrors)
  else:
    discard

proc pushBackendOption(
    optionsStack: var seq[(TOptions, TNoteKinds)], options: TOptions, notes: TNoteKinds
) =
  optionsStack.add (options, notes)

proc popBackendOption(
    config: ConfigRef,
    optionsStack: var seq[(TOptions, TNoteKinds)],
    options: var TOptions,
) =
...
proc processPushBackendOption*(
  ...
  for i in start ..< n.len:
    ...
    if it.kind in nkPragmaCallKinds and it.len == 2:
      if it[0].kind == nkBracketExpr and it[0].len == 2 and it[0][1].kind == nkIdent and
          it[0][0].kind == nkIdent:
