...
proc registerAdditionalOps*(c: PCtx) =
  ...
  when defined(nimcore):
    ...
    registerCallback c,
      "stdlib.staticos.staticWalkDir",
      proc(a: VmArgs) {.nimcall.} =
        setResult(a, staticWalkDirImpl(getString(a, 0), getBool(a, 1)))
    registerCallback c,
      "stdlib.staticos.staticDirExists",
      proc(a: VmArgs) {.nimcall.} =
        setResult(a, dirExists(getString(a, 0)))
    registerCallback c,
      "stdlib.staticos.staticFileExists",
      proc(a: VmArgs) {.nimcall.} =
  ...
  registerCallback c,
    "stdlib.system.ltCStringVm",
    proc(a: VmArgs) =
      setResult(a, getString(a, 0) < getString(a, 1))

  registerCallback c,
    "stdlib.system.leCStringVm",
    proc(a: VmArgs) =
      setResult(a, getString(a, 0) <= getString(a, 1))
