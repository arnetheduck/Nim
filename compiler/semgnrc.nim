    ...
    if candidates.len > 0:
      ...
      if withinBind in flags or s.id in ctx.toBind:
        if s.kind == skType: # don't put types in sym choice
          ...
          result = newDot(
            result,
            semGenericStmtSymbol(
              c, n, s, ctx, flags, isAmbiguous = ambig, fromDotExpr = true
            ),
          )
        else:
...
proc semGenericStmt(c: PContext, n: PNode): PNode =
  var ctx = GenericCtx(toMixin: initIntSet(), toBind: initIntSet())
