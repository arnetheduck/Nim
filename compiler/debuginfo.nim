...
proc register*(self: var DebugInfo, package, file: string): SigHash =
  ...
  for f in self.files:
    if f.mangled == result:
      if f.package == package and f.file == file:
        return
      self.conflicts = true
      break
...
proc hasEnum*(self: DebugInfo, ename: string, id: int, owner: SigHash): bool =
  for en in self.enums:
    if en.owner == owner and en.name == ename and en.id == id:
      return true
...
proc writeDebugInfo*(file: string) =
  writeDebugInfo(gDebugInfo, file)
...
proc loadDebugInfo*(file: string) =
  loadDebugInfo(gDebugInfo, file)
