...
import
  options, ast, trees, nimsets, platform, msgs, idents, renderer, types, commands,
  magicsys, modulegraphs, lineinfos, wordrecg
...
proc isIntRange(t: PType): bool {.inline.} =
  result =
    t.kind == tyRange and t.elementType.kind in {tyInt .. tyInt64, tyUInt8 .. tyUInt32}
  ...
  of mIsNil:
    let val =
      ...
      (
        ...
        a.kind == nkTupleConstr and a.len == 2 and a[0].kind == nkNilLit and
        a[1].kind == nkNilLit
      )
...
proc leValueConv*(a, b: PNode): bool =
  ...
  of nkCharLit .. nkUInt64Lit:
    ...
    of nkFloatLit .. nkFloat128Lit:
      ...
      result = false #internalError(a.info, "leValueConv")
  of nkFloatLit .. nkFloat128Lit:
    case b.kind
    of nkFloatLit .. nkFloat128Lit:
...
proc foldDefine(m, s: PSym, n: PNode, idgen: IdGenerator, g: ModuleGraph): PNode =
  ...
  if isDefined(g.config, name):
    ...
    of mGenericDefine:
      ...
      try:
        ...
        of tyEnum:
          ...
          localError(
            g.config,
            s.info,
            "unsupported type $1 for define '$2'" % [typeToString(rawTyp), name],
          )
      ...
      result = copyTree(s.astdef) # unreachable
  else:
