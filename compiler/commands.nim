...
const
  ...
  errInvalidFeatureButXFound =
    Feature.toSeq
    .map(
      proc(val: Feature): string =
        "'$1'" % $val
    )
    .join(", ") & " expected, but '$1' found"
...
proc testCompileOption*(conf: ConfigRef, switch: string, info: TLineInfo): bool =
  ...
  of "mangle":
    result = contains(conf.globalOptions, optItaniumMangle)
  else:
    result = false
    invalidCmdLineOption(conf, passCmd1, switch, info)
...
proc processSwitch*(
  ...
  of "nimcache":
    ...
    if conf.target.targetOS == osWindows and DirSep == '/':
      arg = arg.replace('\\', '/')
    conf.nimcacheDir = processPath(
      conf, pathRelativeToConfig(arg, pass, conf), info, notRelativeToProj = true
    )
  of "out", "o":
    expectArg(conf, switch, arg, pass, info)
  ...
  of "maxloopiterationsvm":
    ...
    if not value > 0:
      localError(
        conf, info, "maxLoopIterationsVM must be a positive integer greater than zero"
      )
  ...
  of "maxcalldepthvm":
    ...
    if value <= 0:
      localError(
        conf, info, "maxCallDepthVM must be a positive integer greater than zero"
      )
  ...
  of "symbolfiles", "incremental", "ic":
    if switch.normalize == "symbolfiles":
      deprecatedAlias(switch, "incremental") # xxx maybe also ic, since not in help?
    if pass in {passCmd2, passPP}:
      case arg.normalize
      of "on":
