...
proc structOrUnion(t: PType): Snippet =
  ...
  if tfUnion in t.flags: "union" else: "struct"
...
template addStruct(
  ...
  of bcNone:
    ...
    if typ.itemId notin m.g.graph.memberProcsPerType and typ.n != nil and typ.n.len == 1 and
  ...
  body
  if baseKind == bcNone and currLen == obj.len and
      typ.itemId notin m.g.graph.memberProcsPerType:
...
template addFieldWithStructType(
    obj: var Builder, m: BModule, parentTyp: PType, fieldName: string, body: typed
) =
