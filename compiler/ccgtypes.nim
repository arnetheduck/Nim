...
proc fillBackendName(m: BModule, s: PSym) =
  if s.loc.snippet == "":
    var result: Rope
    if s.kind in routineKinds and
        {optCDebug, optItaniumMangle} * m.g.config.globalOptions ==
        {optCDebug, optItaniumMangle} and m.g.config.symbolFiles == disabledSf:
  ...
  of tyEnum:
    if firstOrd(conf, typ) < 0:
      ...
      of 8:
        ...
        result = ctInt32
  of tyRange:
    result = mapType(conf, typ.elementType, isParam)
      ...
      if rettype.isImportedCppType or t.isImportedCppType or
        ...
        result =
          ...
          (t.kind == tyObject and not isObjLackingTypeField(t)) or (
            getSize(conf, rettype) == szUnknownSize and
            (t.sym == nil or sfImportc notin t.sym.flags)
          )
    else:
      result = false
...
proc genMemberProcParams(
    m: BModule,
    prc: PSym,
    superCall, rettype, name, params: var string,
    check: var IntSet,
  ...
  if isCtor or (name[0] == '~' and sfMember in prc.flags):
    ...
    if rettype == "":
      ...
      rettype = runtimeFormat(
        rettype.replace("'0", "$1"), [getTypeDescAux(m, t.returnType, check, dkResult)]
      )
...
proc genCppInitializer(
    m: BModule, prc: BProc, typ: PType, didGenTemp: var bool
): string =
  ...
  of nkSym:
    ...
    if not isImportedCppType(rectype):
      ...
      if fieldType.kind == tyUncheckedArray:
        ...
        let noInit =
          sfNoInit in field.flags or
          (field.typ.sym != nil and sfNoInit in field.typ.sym.flags)
        if not noInit and
            (fieldType.isOrHasImportedCppType() or hasCppCtor(m, field.owner.typ)):
...
proc getTupleDesc(m: BModule, typ: PType, name: Rope, check: var IntSet): Rope =
  ...
  result.addStruct(m, typ, name, ""):
    for i, a in typ.ikids:
      result.addField(name = "Field" & $i, typ = getTypeDescAux(m, a, check, dkField))
...
proc getTypeDescAux(
  ...
  result =
    getTypePre(m, origTyp.skipTypes(irrelevantForBackend - {tyOwned, tyDistinct}), sig)
        ...
        if skipTypes(t.elementType, typedescInst).kind != tyEmpty:
          ...
          if m.compileToCpp:
            appcg(
              ...
              [getTypeDescAux(m, t.elementType, check, kind), result],
            )
          else:
            appcg(
              m,
  ...
  of tyArray:
    ...
    if not isImportedType(t):
      ...
      m.s[cfsTypes].addf("typedef $1 $2[$3];$n", [e, result, rope(n)])
  of tyObject, tyTuple:
...
proc parseVFunctionDecl(
    ...
    isFnConst, isOverride, isMemberVirtual, isStatic: var bool,
    isCtor: bool,
    isFunctor = false,
) =
  ...
  if scanf(val, "$*($*)$s$*", name, params, afterParams):
    if name.strip() == "operator" and params == "": #isFunctor?
      parseVFunctionDecl(
        afterParams, name, params, retType, superCall, isFnConst, isOverride,
        isMemberVirtual, isStatic, isCtor, true,
      )
      return
...
proc genMemberProcHeader(
  ...
  parseVFunctionDecl(
    ...
    isMemberVirtual, isStatic, isCtor,
  )
...
proc genTupleInfo(m: BModule, typ, origType: PType, name: Rope, info: TLineInfo) =
  ...
  if not typ.isEmptyTupleType:
    ...
    m.s[cfsTypeInit3].addf(
      "$1.len = $2; $1.kind = 2; $1.sons = &$3[0];$n", [expr, rope(typ.kidsLen), tmp]
    )
  else:
    m.s[cfsTypeInit3].addf("$1.len = $2; $1.kind = 2;$n", [expr, rope(typ.kidsLen)])
...
proc genDeepCopyProc(m: BModule, s: PSym, result: Rope) =
  genProc(m, s)
  m.s[cfsTypeInit3].addf(
    "$1.deepcopy =(void* (N_RAW_NIMCALL*)(void*))$2;$n", [result, s.loc.snippet]
  )
...
proc genTypeInfoV2OldImpl(m: BModule, t, origType: PType, name: Rope, info: TLineInfo) =
  ...
  if dispatchMethods.len > 0:
    ...
    m.s[cfsVars].addf(
      "static void* $1[$2] = $3;$n",
      [vTablePointerName, rope(dispatchMethods.len), genVTable(dispatchMethods)],
    )
  ...
  if t.kind == tyObject and t.baseClass != nil and
...
proc genTypeInfoV1(m: BModule, t: PType, info: TLineInfo): Rope =
  ...
  of tyStatic:
    if t.n != nil:
      result = genTypeInfoV1(m, skipModifier t, info)
    else:
...
proc genTypeSection(m: BModule, n: PNode) =
  ...
  for i in 0 ..< n.len:
    ...
    for p in 0 ..< n[i][0].len:
      if (n[i][0][p].kind notin {nkSym, nkPostfix}):
        continue
      var s = n[i][0][p]
      if s.kind == nkPostfix:
        s = n[i][0][p][1]
      if {sfExportc, sfCompilerProc} * s.sym.flags == {sfExportc}:
        ...
        if m.g.generatedHeader != nil:
          discard getTypeDescAux(
            m.g.generatedHeader, s.typ, intSet, descKindFromSymKind(s.sym.kind)
          )
