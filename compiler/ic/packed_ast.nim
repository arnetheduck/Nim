...
const nilItemId* = PackedItemId(module: LitId(0), item: 0.int32)
...
template kind*(n: PackedNode): TNodeKind =
  TNodeKind(n.x and NodeKindMask)

template uoperand*(n: PackedNode): uint32 =
  (n.x shr NodeKindBits)
...
template rawSpan(n: PackedNode): int =
  int(uoperand(n))
...
template symId*(n: NodePos): SymId =
  SymId tree.nodes[n.int].soperand
...
const
  externIntLit* = {
    nkCharLit, nkIntLit, nkInt8Lit, nkInt16Lit, nkInt32Lit, nkInt64Lit, nkUIntLit,
